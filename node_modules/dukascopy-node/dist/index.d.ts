import { ValidationRule, ValidationError } from 'fastest-validator';
export { RuleBoolean, RuleDate, RuleNumber, RuleObject, RuleString } from 'fastest-validator';

declare enum Price {
    /**
     * A `bid` is an offer made by an investor, trader, or dealer in an effort to buy a security, commodity, or currency.
     */
    bid = "bid",
    /**
     * The `ask` is the price a seller is willing to accept for a security, commodity, or currency, which is often referred to as the offer price
     */
    ask = "ask"
}
declare type PriceType = keyof typeof Price;

declare enum Timeframe {
    /**
     * Every single price change. No aggregation of OHLC price data
     */
    tick = "tick",
    /**
     *  secondly `(1 second)` aggregation of OHLC price data
     */
    s1 = "s1",
    /**
     *  minutely `(1 minute)` aggregation of OHLC price data
     */
    m1 = "m1",
    /**
     *  5-minute `(5 minutes)` aggregation of OHLC price data
     */
    m5 = "m5",
    /**
     *  15-minute `(15 minutes)` aggregation of OHLC price data
     */
    m15 = "m15",
    /**
     *  half-hour `(30 minutes)` aggregation of OHLC price data
     */
    m30 = "m30",
    /**
     *  hourly `(1 hour)` aggregation of OHLC price data
     */
    h1 = "h1",
    /**
     *  4-hour `(4 hour)` aggregation of OHLC price data
     */
    h4 = "h4",
    /**
     *  daily `(1 day)` aggregation of OHLC price data
     */
    d1 = "d1",
    /**
     *  monthly `(1 month)` aggregation of OHLC price data
     */
    mn1 = "mn1"
}
declare type TimeframeType = keyof typeof Timeframe;

declare enum Format {
    /**
     * Data will be returned in a shape of array of arrays of numbers
     * ***
     * `m1`, `m30`, `h1`, `d1`, `mn1`:
     * ```
     * // t-stamp open  high  low   close volume
     * [[1000000, 1.01, 1.02, 1.03, 1.04, 1111],
     * [20000000, 2.01, 2.02, 2.03, 2.04, 2222],
     * [30000000, 3.01, 3.02, 3.03, 3.04, 3333]]
     * ```
     * ***
     * `tick`:
     * ```
     * // t-stamp ask   bid   askV. bidV.
     * [[1000000, 1.01, 1.02, 1111, 9999],
     * [20000000, 2.01, 2.02, 2222, 8888],
     * [30000000, 3.01, 3.02, 3333, 7777]]
     * ```
     */
    array = "array",
    /**
     * Data will be returned as **JSON**
     * ***
     * `m1`, `m30`, `h1`, `d1`, `mn1`:
     * ```
     * {
     *   timestamp: 1000000000000,
     *   open: 1.00001,
     *   high: 1.00002,
     *   low: 1.00003,
     *   close: 1.00004,
     *   volume: 1111
     * }
     * ```
     *  ***
     * `tick`:
     * ```
     * {
     *  timestamp: 1000000000000,
     *  askPrice: 1.00001,
     *  bidPrice: 1.00002,
     *  askVolume: 1111,
     *  bidVolume: 9999
     * }
     * ```
     */
    json = "json",
    /**
     * Data will be returned as a **CSV** string
     * ***
     * `m1`, `m30`, `h1`, `d1`, `mn1`:
     * ```
     * `
     * timestamp,open,high,low,close,volume
     * 10000000,1.01,1.02,1.03,1.04,1111,
     * 20000000,2.01,2.02,2.03,2.04,2222,
     * 30000000,3.01,3.02,3.03,3.04,3333
     * `
     * ```
     * ***
     * `tick`:
     * ```
     * `
     * timestamp,askPrice,bidPrice,askVolume,bidVolume
     * 10000000,1.01,1.02,1111,9999
     * 20000000,2.01,2.02,2222,8888
     * 30000000,3.01,3.02,3333,7777
     * `
     * ```
     */
    csv = "csv"
}
declare type FormatType = keyof typeof Format;

declare enum Instrument {
    /**
     * ### HSBC Holdings
     * *0005.HK/HKD*
     */
    '0005hkhkd' = "0005hkhkd",
    /**
     * ### Galaxy Entertainment Group
     * *0027.HK/HKD*
     */
    '0027hkhkd' = "0027hkhkd",
    /**
     * ### Geely Automobile Holdings
     * *0175.HK/HKD*
     */
    '0175hkhkd' = "0175hkhkd",
    /**
     * ### China Resources Beer Holdings
     * *0291.HK/HKD*
     */
    '0291hkhkd' = "0291hkhkd",
    /**
     * ### Sinopec Corp
     * *0386.HK/HKD*
     */
    '0386hkhkd' = "0386hkhkd",
    /**
     * ### HK Exchanges & Clearing
     * *0388.HK/HKD*
     */
    '0388hkhkd' = "0388hkhkd",
    /**
     * ### Tencent Holdings
     * *0700.HK/HKD*
     */
    '0700hkhkd' = "0700hkhkd",
    /**
     * ### PetroChina
     * *0857.HK/HKD*
     */
    '0857hkhkd' = "0857hkhkd",
    /**
     * ### China National Offshore Oil Corporation
     * *0883.HK/HKD*
     */
    '0883hkhkd' = "0883hkhkd",
    /**
     * ### China Construction Bank
     * *0939.HK/HKD*
     */
    '0939hkhkd' = "0939hkhkd",
    /**
     * ### China Mobile
     * *0941.HK/HKD*
     */
    '0941hkhkd' = "0941hkhkd",
    /**
     * ### CITIC Bank International
     * *0998.HK/HKD*
     */
    '0998hkhkd' = "0998hkhkd",
    /**
     * ### CSPC Pharmaceutical Group
     * *1093.HK/HKD*
     */
    '1093hkhkd' = "1093hkhkd",
    /**
     * ### Sino Biopharmeceutical
     * *1177.HK/HKD*
     */
    '1177hkhkd' = "1177hkhkd",
    /**
     * ### Agricultural Bank of China
     * *1288.HK/HKD*
     */
    '1288hkhkd' = "1288hkhkd",
    /**
     * ### AIA
     * *1299.HK/HKD*
     */
    '1299hkhkd' = "1299hkhkd",
    /**
     * ### Industrial and Commercial Bank of China
     * *1398.HK/HKD*
     */
    '1398hkhkd' = "1398hkhkd",
    /**
     * ### Xiaomi Corp
     * *1810.HK/HKD*
     */
    '1810hkhkd' = "1810hkhkd",
    /**
     * ### Sunac China Holdings
     * *1918.HK/HKD*
     */
    '1918hkhkd' = "1918hkhkd",
    /**
     * ### Country Garden Holdings
     * *2007.HK/HKD*
     */
    '2007hkhkd' = "2007hkhkd",
    /**
     * ### AAC Technologies Holdings
     * *2018.HK/HKD*
     */
    '2018hkhkd' = "2018hkhkd",
    /**
     * ### Ping An Insurance
     * *2318.HK/HKD*
     */
    '2318hkhkd' = "2318hkhkd",
    /**
     * ### BOC Hong Kong (Holdings)
     * *2388.HK/HKD*
     */
    '2388hkhkd' = "2388hkhkd",
    /**
     * ### Asahi Group Holdings
     * *2502.JP/JPY*
     */
    '2502jpjpy' = "2502jpjpy",
    /**
     * ### Kirin Brewery Co
     * *2503.JP/JPY*
     */
    '2503jpjpy' = "2503jpjpy",
    /**
     * ### China Life Insurance Company
     * *2628.HK/HKD*
     */
    '2628hkhkd' = "2628hkhkd",
    /**
     * ### CSOP FTSE China A50 ETF
     * *2822.HK/HKD*
     */
    '2822hkhkd' = "2822hkhkd",
    /**
     * ### Hang Seng H-Share Index ETF
     * *2828.HK/HKD*
     */
    '2828hkhkd' = "2828hkhkd",
    /**
     * ### iShares Core S&P BSE SENSEX India ETF
     * *2836.HK/HKD*
     */
    '2836hkhkd' = "2836hkhkd",
    /**
     * ### Japan Tobacco
     * *2914.JP/JPY*
     */
    '2914jpjpy' = "2914jpjpy",
    /**
     * ### ChinaAMC CSI 300 Index ETF
     * *3188.HK/HKD*
     */
    '3188hkhkd' = "3188hkhkd",
    /**
     * ### China Evergrande Group
     * *3333.HK/HKD*
     */
    '3333hkhkd' = "3333hkhkd",
    /**
     * ### Seven and I Holdings
     * *3382.JP/JPY*
     */
    '3382jpjpy' = "3382jpjpy",
    /**
     * ### Sumco Corp
     * *3436.JP/JPY*
     */
    '3436jpjpy' = "3436jpjpy",
    /**
     * ### China Merchants Bank
     * *3968.HK/HKD*
     */
    '3968hkhkd' = "3968hkhkd",
    /**
     * ### Bank of China
     * *3988.HK/HKD*
     */
    '3988hkhkd' = "3988hkhkd",
    /**
     * ### Showa Denko
     * *4004.JP/JPY*
     */
    '4004jpjpy' = "4004jpjpy",
    /**
     * ### Sumitomo Chemical
     * *4005.JP/JPY*
     */
    '4005jpjpy' = "4005jpjpy",
    /**
     * ### Shin-Etsu Chemical
     * *4063.JP/JPY*
     */
    '4063jpjpy' = "4063jpjpy",
    /**
     * ### Kao Corp
     * *4452.JP/JPY*
     */
    '4452jpjpy' = "4452jpjpy",
    /**
     * ### Takeda
     * *4502.JP/JPY*
     */
    '4502jpjpy' = "4502jpjpy",
    /**
     * ### Astellas Pharma
     * *4503.JP/JPY*
     */
    '4503jpjpy' = "4503jpjpy",
    /**
     * ### Shionogi
     * *4507.JP/JPY*
     */
    '4507jpjpy' = "4507jpjpy",
    /**
     * ### Eisai
     * *4523.JP/JPY*
     */
    '4523jpjpy' = "4523jpjpy",
    /**
     * ### Terumo Corp
     * *4543.JP/JPY*
     */
    '4543jpjpy' = "4543jpjpy",
    /**
     * ### Yahoo Japan Corp
     * *4689.JP/JPY*
     */
    '4689jpjpy' = "4689jpjpy",
    /**
     * ### Shiseido
     * *4911.JP/JPY*
     */
    '4911jpjpy' = "4911jpjpy",
    /**
     * ### Bridgestone Corp
     * *5108.JP/JPY*
     */
    '5108jpjpy' = "5108jpjpy",
    /**
     * ### Tokai Carbon
     * *5301.JP/JPY*
     */
    '5301jpjpy' = "5301jpjpy",
    /**
     * ### Nippon Steel Corp
     * *5401.JP/JPY*
     */
    '5401jpjpy' = "5401jpjpy",
    /**
     * ### Recruit Holdings Co Ltd
     * *6098.JP/JPY*
     */
    '6098jpjpy' = "6098jpjpy",
    /**
     * ### Komatsu
     * *6301.JP/JPY*
     */
    '6301jpjpy' = "6301jpjpy",
    /**
     * ### Daikin Industries
     * *6367.JP/JPY*
     */
    '6367jpjpy' = "6367jpjpy",
    /**
     * ### Hitachi
     * *6501.JP/JPY*
     */
    '6501jpjpy' = "6501jpjpy",
    /**
     * ### Toshiba Corp
     * *6502.JP/JPY*
     */
    '6502jpjpy' = "6502jpjpy",
    /**
     * ### Mitsubishi Electric Corp
     * *6503.JP/JPY*
     */
    '6503jpjpy' = "6503jpjpy",
    /**
     * ### Yaskawa Electric Corp
     * *6506.JP/JPY*
     */
    '6506jpjpy' = "6506jpjpy",
    /**
     * ### Fujitsu
     * *6702.JP/JPY*
     */
    '6702jpjpy' = "6702jpjpy",
    /**
     * ### Panasonic Corp
     * *6752.JP/JPY*
     */
    '6752jpjpy' = "6752jpjpy",
    /**
     * ### Sony Corp Japan
     * *6758.JP/JPY*
     */
    '6758jpjpy' = "6758jpjpy",
    /**
     * ### TDK Corp
     * *6762.JP/JPY*
     */
    '6762jpjpy' = "6762jpjpy",
    /**
     * ### Denso Corp
     * *6902.JP/JPY*
     */
    '6902jpjpy' = "6902jpjpy",
    /**
     * ### Fanuc
     * *6954.JP/JPY*
     */
    '6954jpjpy' = "6954jpjpy",
    /**
     * ### Kyocera
     * *6971.JP/JPY*
     */
    '6971jpjpy' = "6971jpjpy",
    /**
     * ### Nissan Motor Co
     * *7201.JP/JPY*
     */
    '7201jpjpy' = "7201jpjpy",
    /**
     * ### Toyota Motor Corp
     * *7203.JP/JPY*
     */
    '7203jpjpy' = "7203jpjpy",
    /**
     * ### Mazda Motor
     * *7261.JP/JPY*
     */
    '7261jpjpy' = "7261jpjpy",
    /**
     * ### Honda Motor Company
     * *7267.JP/JPY*
     */
    '7267jpjpy' = "7267jpjpy",
    /**
     * ### Suzuki Motor Corp
     * *7269.JP/JPY*
     */
    '7269jpjpy' = "7269jpjpy",
    /**
     * ### Subaru Corporation
     * *7270.JP/JPY*
     */
    '7270jpjpy' = "7270jpjpy",
    /**
     * ### Canon
     * *7751.JP/JPY*
     */
    '7751jpjpy' = "7751jpjpy",
    /**
     * ### Nintendo Co Ltd
     * *7974.JP/JPY*
     */
    '7974jpjpy' = "7974jpjpy",
    /**
     * ### Mitsui and Company
     * *8031.JP/JPY*
     */
    '8031jpjpy' = "8031jpjpy",
    /**
     * ### Tokyo Electron
     * *8035.JP/JPY*
     */
    '8035jpjpy' = "8035jpjpy",
    /**
     * ### Mitsubishi UFJ Financial Group
     * *8306.JP/JPY*
     */
    '8306jpjpy' = "8306jpjpy",
    /**
     * ### Sumitomo Mitsui Financial Group
     * *8316.JP/JPY*
     */
    '8316jpjpy' = "8316jpjpy",
    /**
     * ### Mizuho Financial Group
     * *8411.JP/JPY*
     */
    '8411jpjpy' = "8411jpjpy",
    /**
     * ### Tokio Marine Holdings
     * *8766.JP/JPY*
     */
    '8766jpjpy' = "8766jpjpy",
    /**
     * ### Mitsui Fudosan Company
     * *8801.JP/JPY*
     */
    '8801jpjpy' = "8801jpjpy",
    /**
     * ### Mitsubishi Estate Company
     * *8802.JP/JPY*
     */
    '8802jpjpy' = "8802jpjpy",
    /**
     * ### East Japan Railway Company
     * *9020.JP/JPY*
     */
    '9020jpjpy' = "9020jpjpy",
    /**
     * ### Nippon Telegraph and Tel
     * *9432.JP/JPY*
     */
    '9432jpjpy' = "9432jpjpy",
    /**
     * ### KDDI Corp
     * *9433.JP/JPY*
     */
    '9433jpjpy' = "9433jpjpy",
    /**
     * ### Tokyo Electric Power Company
     * *9501.JP/JPY*
     */
    '9501jpjpy' = "9501jpjpy",
    /**
     * ### Fast Retailing
     * *9983.JP/JPY*
     */
    '9983jpjpy' = "9983jpjpy",
    /**
     * ### Softbank Group Corp
     * *9984.JP/JPY*
     */
    '9984jpjpy' = "9984jpjpy",
    /**
     * ### AGILENT TECHNOLOGIES INC
     * *A.US/USD*
     */
    aususd = "aususd",
    /**
     * ### A2A Spa
     * *A2A.IT/EUR*
     */
    a2aiteur = "a2aiteur",
    /**
     * ### ALCOA INC
     * *AA.US/USD*
     */
    aaususd = "aaususd",
    /**
     * ### AbbVie Inc
     * *AABV.US/USD*
     */
    aabvususd = "aabvususd",
    /**
     * ### Anglo American PLC
     * *AAL.GB/GBX*
     */
    aalgbgbx = "aalgbgbx",
    /**
     * ### AMERICAN AIRLINES GROUP INC
     * *AAL.US/USD*
     */
    aalususd = "aalususd",
    /**
     * ### ADVANCE AUTO PARTS INC
     * *AAP.US/USD*
     */
    aapususd = "aapususd",
    /**
     * ### APPLE INC
     * *AAPL.US/USD*
     */
    aaplususd = "aaplususd",
    /**
     * ### ABB Ltd
     * *ABB.SE/SEK*
     */
    abbsesek = "abbsesek",
    /**
     * ### ABB Ltd
     * *ABBN.CH/CHF*
     */
    abbnchchf = "abbnchchf",
    /**
     * ### AMERISOURCEBERGEN CORP
     * *ABC.US/USD*
     */
    abcususd = "abcususd",
    /**
     * ### Ambev S.A.
     * *ABEV.US/USD*
     */
    abevususd = "abevususd",
    /**
     * ### Associated British Foods PLC
     * *ABF.GB/GBX*
     */
    abfgbgbx = "abfgbgbx",
    /**
     * ### Anheuser-Busch InBev NV
     * *ABI.BE/EUR*
     */
    abibeeur = "abibeeur",
    /**
     * ### ABIOMED INC
     * *ABMD.US/USD*
     */
    abmdususd = "abmdususd",
    /**
     * ### ABBOTT LABORATORIES
     * *ABT.US/USD*
     */
    abtususd = "abtususd",
    /**
     * ### Accor SA
     * *AC.FR/EUR*
     */
    acfreur = "acfreur",
    /**
     * ### Credit Agricole SA
     * *ACA.FR/EUR*
     */
    acafreur = "acafreur",
    /**
     * ### ARCH CAPITAL GROUP LTD
     * *ACGL.US/USD*
     */
    acglususd = "acglususd",
    /**
     * ### AECOM
     * *ACM.US/USD*
     */
    acmususd = "acmususd",
    /**
     * ### ACCENTURE PLC-CL A
     * *ACN.US/USD*
     */
    acnususd = "acnususd",
    /**
     * ### ACS Actividades de Construccion y Servicios SA
     * *ACS.ES/EUR*
     */
    acseseur = "acseseur",
    /**
     * ### Acerinox SA
     * *ACX.ES/EUR*
     */
    acxeseur = "acxeseur",
    /**
     * ### Cardano vs US Dollar
     * *ADA/USD*
     */
    adausd = "adausd",
    /**
     * ### ADOBE SYSTEMS INC
     * *ADBE.US/USD*
     */
    adbeususd = "adbeususd",
    /**
     * ### Adecco SA
     * *ADEN.CH/CHF*
     */
    adenchchf = "adenchchf",
    /**
     * ### ANALOG DEVICES INC
     * *ADI.US/USD*
     */
    adiususd = "adiususd",
    /**
     * ### Admiral Group PLC
     * *ADM.GB/GBX*
     */
    admgbgbx = "admgbgbx",
    /**
     * ### AUTOMATIC DATA PROCESSING
     * *ADP.US/USD*
     */
    adpususd = "adpususd",
    /**
     * ### Adidas AG
     * *ADS.DE/EUR*
     */
    adsdeeur = "adsdeeur",
    /**
     * ### AUTODESK INC
     * *ADSK.US/USD*
     */
    adskususd = "adskususd",
    /**
     * ### Aena SA
     * *AENA.ES/EUR*
     */
    aenaeseur = "aenaeseur",
    /**
     * ### AMERICAN ELECTRIC POWER
     * *AEP.US/USD*
     */
    aepususd = "aepususd",
    /**
     * ### Air France-KLM
     * *AF.FR/EUR*
     */
    affreur = "affreur",
    /**
     * ### AMERICAN FINANCIAL GROUP INC
     * *AFG.US/USD*
     */
    afgususd = "afgususd",
    /**
     * ### Aggreko PLC
     * *AGK.GB/GBX*
     */
    agkgbgbx = "agkgbgbx",
    /**
     * ### Autogrill Spa
     * *AGL.IT/EUR*
     */
    agliteur = "agliteur",
    /**
     * ### Aegon NV
     * *AGN.NL/EUR*
     */
    agnnleur = "agnnleur",
    /**
     * ### AGNC INVESTMENT CORP
     * *AGNC.US/USD*
     */
    agncususd = "agncususd",
    /**
     * ### Ageas
     * *AGS.BE/EUR*
     */
    agsbeeur = "agsbeeur",
    /**
     * ### Koninklijke Ahold Delhaize NV
     * *AH.NL/EUR*
     */
    ahnleur = "ahnleur",
    /**
     * ### Ashtead Group PLC
     * *AHT.GB/GBX*
     */
    ahtgbgbx = "ahtgbgbx",
    /**
     * ### Air Liquide SA
     * *AI.FR/EUR*
     */
    aifreur = "aifreur",
    /**
     * ### AMERICAN INTERNATIONAL GROUP
     * *AIG.US/USD*
     */
    aigususd = "aigususd",
    /**
     * ### Airbus Group SE
     * *AIR.FR/EUR*
     */
    airfreur = "airfreur",
    /**
     * ### ARTHUR J GALLAGHER & CO
     * *AJG.US/USD*
     */
    ajgususd = "ajgususd",
    /**
     * ### Akzo Nobel NV
     * *AKZA.NL/EUR*
     */
    akzanleur = "akzanleur",
    /**
     * ### ALBEMARLE CORP
     * *ALB.US/USD*
     */
    albususd = "albususd",
    /**
     * ### Alfa Laval AB
     * *ALFA.SE/SEK*
     */
    alfasesek = "alfasesek",
    /**
     * ### Alfa SA B A
     * *ALFAA.MX/MXN*
     */
    alfaamxmxn = "alfaamxmxn",
    /**
     * ### ALIGN TECHNOLOGY INC
     * *ALGN.US/USD*
     */
    algnususd = "algnususd",
    /**
     * ### ALASKA AIR GROUP INC
     * *ALK.US/USD*
     */
    alkususd = "alkususd",
    /**
     * ### ALKERMES PLC
     * *ALKS.US/USD*
     */
    alksususd = "alksususd",
    /**
     * ### ALLSTATE CORP
     * *ALL.US/USD*
     */
    allususd = "allususd",
    /**
     * ### ALLEGION PLC
     * *ALLE.US/USD*
     */
    alleususd = "alleususd",
    /**
     * ### ALLY FINANCIAL INC
     * *ALLY.US/USD*
     */
    allyususd = "allyususd",
    /**
     * ### ALNYLAM PHARMACEUTICALS INC
     * *ALNY.US/USD*
     */
    alnyususd = "alnyususd",
    /**
     * ### Alstom SA
     * *ALO.FR/EUR*
     */
    alofreur = "alofreur",
    /**
     * ### Alsea SAB de CV
     * *ALSEA.MX/MXN*
     */
    alseamxmxn = "alseamxmxn",
    /**
     * ### Allianz SE
     * *ALV.DE/EUR*
     */
    alvdeeur = "alvdeeur",
    /**
     * ### ALEXION PHARMACEUTICALS INC
     * *ALXN.US/USD*
     */
    alxnususd = "alxnususd",
    /**
     * ### APPLIED MATERIALS INC
     * *AMAT.US/USD*
     */
    amatususd = "amatususd",
    /**
     * ### AMCOR PLC
     * *AMCR.US/USD*
     */
    amcrususd = "amcrususd",
    /**
     * ### ADVANCED MICRO DEVICES
     * *AMD.US/USD*
     */
    amdususd = "amdususd",
    /**
     * ### AMGEN INC
     * *AMGN.US/USD*
     */
    amgnususd = "amgnususd",
    /**
     * ### AMERICAN HOMES 4 RENT- A
     * *AMH.US/USD*
     */
    amhususd = "amhususd",
    /**
     * ### Amplifon Spa
     * *AMP.IT/EUR*
     */
    ampiteur = "ampiteur",
    /**
     * ### AMERIPRISE FINANCIAL INC
     * *AMP.US/USD*
     */
    ampususd = "ampususd",
    /**
     * ### Amadeus IT Holding SA
     * *AMS.ES/EUR*
     */
    amseseur = "amseseur",
    /**
     * ### AMERICAN TOWER CORP
     * *AMT.US/USD*
     */
    amtususd = "amtususd",
    /**
     * ### American Well Corp Class A
     * *AMWL.US/USD*
     */
    amwlususd = "amwlususd",
    /**
     * ### America Movil SAB de C SERL
     * *AMXL.MX/MXN*
     */
    amxlmxmxn = "amxlmxmxn",
    /**
     * ### AMAZON.COM INC
     * *AMZN.US/USD*
     */
    amznususd = "amznususd",
    /**
     * ### ARISTA NETWORKS INC
     * *ANET.US/USD*
     */
    anetususd = "anetususd",
    /**
     * ### ANSYS INC
     * *ANSS.US/USD*
     */
    anssususd = "anssususd",
    /**
     * ### ANTHEM INC
     * *ANTM.US/USD*
     */
    antmususd = "antmususd",
    /**
     * ### Antofagasta PLC
     * *ANTO.GB/GBX*
     */
    antogbgbx = "antogbgbx",
    /**
     * ### AON PLC-CLASS A
     * *AON.US/USD*
     */
    aonususd = "aonususd",
    /**
     * ### SMITH (A.O.) CORP
     * *AOS.US/USD*
     */
    aosususd = "aosususd",
    /**
     * ### APACHE CORP
     * *APA.US/USD*
     */
    apaususd = "apaususd",
    /**
     * ### AIR PRODUCTS & CHEMICALS INC
     * *APD.US/USD*
     */
    apdususd = "apdususd",
    /**
     * ### Applus Services
     * *APP.ES/EUR*
     */
    appeseur = "appeseur",
    /**
     * ### APTIV PLC
     * *APTV.US/USD*
     */
    aptvususd = "aptvususd",
    /**
     * ### ANTERO RESOURCES CORP
     * *AR.US/USD*
     */
    arususd = "arususd",
    /**
     * ### Arca Continental SAB de CV
     * *ARCA.MX/MXN*
     */
    arcamxmxn = "arcamxmxn",
    /**
     * ### ALEXANDRIA REAL ESTATE EQUIT
     * *ARE.US/USD*
     */
    areususd = "areususd",
    /**
     * ### ARAMARK
     * *ARMK.US/USD*
     */
    armkususd = "armkususd",
    /**
     * ### ARROW ELECTRONICS INC
     * *ARW.US/USD*
     */
    arwususd = "arwususd",
    /**
     * ### ASML Holding NV
     * *ASML.NL/EUR*
     */
    asmlnleur = "asmlnleur",
    /**
     * ### Grupo Aeroportuario del Sureste
     * *ASURB.MX/MXN*
     */
    asurbmxmxn = "asurbmxmxn",
    /**
     * ### Atlas Copco AB
     * *ATCOA.SE/SEK*
     */
    atcoasesek = "atcoasesek",
    /**
     * ### Atlantia Spa
     * *ATL.IT/EUR*
     */
    atliteur = "atliteur",
    /**
     * ### ATOS SE
     * *ATO.FR/EUR*
     */
    atofreur = "atofreur",
    /**
     * ### ATMOS ENERGY CORP
     * *ATO.US/USD*
     */
    atoususd = "atoususd",
    /**
     * ### ACTIVISION BLIZZARD INC
     * *ATVI.US/USD*
     */
    atviususd = "atviususd",
    /**
     * ### Australian Dollar vs Canadian Dollar
     * *AUD/CAD*
     */
    audcad = "audcad",
    /**
     * ### Australian Dollar vs Swiss Franc
     * *AUD/CHF*
     */
    audchf = "audchf",
    /**
     * ### Australian Dollar vs Japanese Yen
     * *AUD/JPY*
     */
    audjpy = "audjpy",
    /**
     * ### Australian Dollar vs New Zealand Dollar
     * *AUD/NZD*
     */
    audnzd = "audnzd",
    /**
     * ### Australian Dollar vs Singapore Dollar
     * *AUD/SGD*
     */
    audsgd = "audsgd",
    /**
     * ### Australian Dollar vs US Dollar
     * *AUD/USD*
     */
    audusd = "audusd",
    /**
     * ### Aviva PLC
     * *AV.GB/GBX*
     */
    avgbgbx = "avgbgbx",
    /**
     * ### AVALONBAY COMMUNITIES INC
     * *AVB.US/USD*
     */
    avbususd = "avbususd",
    /**
     * ### Aave vs US Dollar
     * *AVE/USD*
     */
    aveusd = "aveusd",
    /**
     * ### Broadcom Limited
     * *AVGO.US/USD*
     */
    avgoususd = "avgoususd",
    /**
     * ### AVALARA INC
     * *AVLR.US/USD*
     */
    avlrususd = "avlrususd",
    /**
     * ### AVANTOR INC
     * *AVTR.US/USD*
     */
    avtrususd = "avtrususd",
    /**
     * ### AVEVA GROUP PLC
     * *AVV.GB/GBX*
     */
    avvgbgbx = "avvgbgbx",
    /**
     * ### AMERICAN WATER WORKS CO INC
     * *AWK.US/USD*
     */
    awkususd = "awkususd",
    /**
     * ### AMERICAN EXPRESS CO
     * *AXP.US/USD*
     */
    axpususd = "axpususd",
    /**
     * ### AXALTA COATING SYSTEMS LTD
     * *AXTA.US/USD*
     */
    axtaususd = "axtaususd",
    /**
     * ### ALTERYX INC - CLASS A
     * *AYX.US/USD*
     */
    ayxususd = "ayxususd",
    /**
     * ### Azimut Holding Spa
     * *AZM.IT/EUR*
     */
    azmiteur = "azmiteur",
    /**
     * ### AstraZeneca PLC
     * *AZN.GB/GBX*
     */
    azngbgbx = "azngbgbx",
    /**
     * ### AstraZeneca PLC
     * *AZN.US/USD*
     */
    aznususd = "aznususd",
    /**
     * ### AUTOZONE INC
     * *AZO.US/USD*
     */
    azoususd = "azoususd",
    /**
     * ### ASPEN TECHNOLOGY INC
     * *AZPN.US/USD*
     */
    azpnususd = "azpnususd",
    /**
     * ### BAE Systems PLC
     * *BA.GB/GBX*
     */
    bagbgbx = "bagbgbx",
    /**
     * ### Boeing Co
     * *BA.US/USD*
     */
    baususd = "baususd",
    /**
     * ### Babcock International Group PLC
     * *BAB.GB/GBX*
     */
    babgbgbx = "babgbgbx",
    /**
     * ### ALIBABA GROUP HOLDING-SP ADR
     * *BABA.US/USD*
     */
    babaususd = "babaususd",
    /**
     * ### BANK OF AMERICA CORP
     * *BAC.US/USD*
     */
    bacususd = "bacususd",
    /**
     * ### Julius Baer Group Ltd
     * *BAER.CH/CHF*
     */
    baerchchf = "baerchchf",
    /**
     * ### BOOZ ALLEN HAMILTON HOLDINGS
     * *BAH.US/USD*
     */
    bahususd = "bahususd",
    /**
     * ### Banco BPM Spa
     * *BAMI.IT/EUR*
     */
    bamiiteur = "bamiiteur",
    /**
     * ### Barclays PLC
     * *BARC.GB/GBX*
     */
    barcgbgbx = "barcgbgbx",
    /**
     * ### BASF SE
     * *BAS.DE/EUR*
     */
    basdeeur = "basdeeur",
    /**
     * ### Basic Attention Token vs US Dollar
     * *BAT/USD*
     */
    batusd = "batusd",
    /**
     * ### British American Tobacco PLC
     * *BATS.GB/GBX*
     */
    batsgbgbx = "batsgbgbx",
    /**
     * ### Bayer AG
     * *BAYN.DE/EUR*
     */
    bayndeeur = "bayndeeur",
    /**
     * ### Banco Del Bajio SA
     * *BBAJIOO.MX/MXN*
     */
    bbajioomxmxn = "bbajioomxmxn",
    /**
     * ### Banco Bradesco S.A.
     * *BBD.US/USD*
     */
    bbdususd = "bbdususd",
    /**
     * ### Banco Bilbao Vizcaya Argentaria SA
     * *BBVA.ES/EUR*
     */
    bbvaeseur = "bbvaeseur",
    /**
     * ### BATH & BODY WORKS INC
     * *BBWI.US/USD*
     */
    bbwiususd = "bbwiususd",
    /**
     * ### BEST BUY CO INC
     * *BBY.US/USD*
     */
    bbyususd = "bbyususd",
    /**
     * ### Brunello Cucinelli Spa
     * *BC.IT/EUR*
     */
    bciteur = "bciteur",
    /**
     * ### Bitcoin Cash vs US dollar
     * *BCH/USD*
     */
    bchusd = "bchusd",
    /**
     * ### BARRATT DEVELOPMENTS PLC
     * *BDEV.GB/GBX*
     */
    bdevgbgbx = "bdevgbgbx",
    /**
     * ### BECTON DICKINSON AND CO
     * *BDX.US/USD*
     */
    bdxususd = "bdxususd",
    /**
     * ### Beiersdorf AG
     * *BEI.DE/EUR*
     */
    beideeur = "beideeur",
    /**
     * ### Proximus
     * *BELG.BE/EUR*
     */
    belgbeeur = "belgbeeur",
    /**
     * ### BERRY GLOBAL GROUP INC
     * *BERY.US/USD*
     */
    beryususd = "beryususd",
    /**
     * ### BUNGE LTD
     * *BG.US/USD*
     */
    bgususd = "bgususd",
    /**
     * ### BAIDU INC
     * *BIDU.US/USD*
     */
    biduususd = "biduususd",
    /**
     * ### BIOGEN INC
     * *BIIB.US/USD*
     */
    biibususd = "biibususd",
    /**
     * ### BIO-RAD LABORATORIES-A
     * *BIO.US/USD*
     */
    bioususd = "bioususd",
    /**
     * ### Bank Of Ireland Plc
     * *BIRG.IE/EUR*
     */
    birgieeur = "birgieeur",
    /**
     * ### BANK OF NEW YORK MELLON CORP
     * *BK.US/USD*
     */
    bkususd = "bkususd",
    /**
     * ### BLACK KNIGHT INC
     * *BKI.US/USD*
     */
    bkiususd = "bkiususd",
    /**
     * ### BAKER HUGHES CO
     * *BKR.US/USD*
     */
    bkrususd = "bkrususd",
    /**
     * ### BLACKROCK INC
     * *BLK.US/USD*
     */
    blkususd = "blkususd",
    /**
     * ### British Land Co PLC
     * *BLND.GB/GBX*
     */
    blndgbgbx = "blndgbgbx",
    /**
     * ### BHP Billiton PLC
     * *BLT.GB/GBX*
     */
    bltgbgbx = "bltgbgbx",
    /**
     * ### Banca Monte dei Paschi di Siena Spa
     * *BMPS.IT/EUR*
     */
    bmpsiteur = "bmpsiteur",
    /**
     * ### BIOMARIN PHARMACEUTICAL INC
     * *BMRN.US/USD*
     */
    bmrnususd = "bmrnususd",
    /**
     * ### Bayerische Motoren Werke AG
     * *BMW.DE/EUR*
     */
    bmwdeeur = "bmwdeeur",
    /**
     * ### BRISTOL-MYERS SQUIBB CO
     * *BMY.US/USD*
     */
    bmyususd = "bmyususd",
    /**
     * ### Danone SA
     * *BN.FR/EUR*
     */
    bnfreur = "bnfreur",
    /**
     * ### BNP Paribas SA
     * *BNP.FR/EUR*
     */
    bnpfreur = "bnpfreur",
    /**
     * ### BRENNTAG SE
     * *BNR.DE/EUR*
     */
    bnrdeeur = "bnrdeeur",
    /**
     * ### Bunzl PLC
     * *BNZL.GB/GBX*
     */
    bnzlgbgbx = "bnzlgbgbx",
    /**
     * ### Bolsa Mexicana de Valores SAB de CV
     * *BOLSAA.MX/MXN*
     */
    bolsaamxmxn = "bolsaamxmxn",
    /**
     * ### HUGO BOSS AG
     * *BOSS.DE/EUR*
     */
    bossdeeur = "bossdeeur",
    /**
     * ### BP PLC
     * *BP.GB/GBX*
     */
    bpgbgbx = "bpgbgbx",
    /**
     * ### BP p.l.c.
     * *BP.US/USD*
     */
    bpususd = "bpususd",
    /**
     * ### Bper Banca Spa
     * *BPE.IT/EUR*
     */
    bpeiteur = "bpeiteur",
    /**
     * ### POPULAR INC
     * *BPOP.US/USD*
     */
    bpopususd = "bpopususd",
    /**
     * ### BROADRIDGE FINANCIAL SOLUTIO
     * *BR.US/USD*
     */
    brususd = "brususd",
    /**
     * ### Burberry Group PLC
     * *BRBY.GB/GBX*
     */
    brbygbgbx = "brbygbgbx",
    /**
     * ### Brembo Spa
     * *BRE.IT/EUR*
     */
    breiteur = "breiteur",
    /**
     * ### BERKSHIRE HATHAWAY INC-CL B
     * *BRKB.US/USD*
     */
    brkbususd = "brkbususd",
    /**
     * ### BRUKER CORP
     * *BRKR.US/USD*
     */
    brkrususd = "brkrususd",
    /**
     * ### BROWN & BROWN INC
     * *BRO.US/USD*
     */
    broususd = "broususd",
    /**
     * ### BRIXMOR PROPERTY GROUP INC
     * *BRX.US/USD*
     */
    brxususd = "brxususd",
    /**
     * ### BOSTON SCIENTIFIC CORP
     * *BSX.US/USD*
     */
    bsxususd = "bsxususd",
    /**
     * ### BT Group PLC
     * *BT.GB/GBX*
     */
    btgbgbx = "btgbgbx",
    /**
     * ### Bitcoin vs US Dollar
     * *BTC/USD*
     */
    btcusd = "btcusd",
    /**
     * ### Euro Bund
     * *BUND.TR/EUR*
     */
    bundtreur = "bundtreur",
    /**
     * ### BURLINGTON STORES INC
     * *BURL.US/USD*
     */
    burlususd = "burlususd",
    /**
     * ### BORGWARNER INC
     * *BWA.US/USD*
     */
    bwaususd = "bwaususd",
    /**
     * ### BEYOND MEAT INC
     * *BYND.US/USD*
     */
    byndususd = "byndususd",
    /**
     * ### Buzzi Unicem Spa
     * *BZU.IT/EUR*
     */
    bzuiteur = "bzuiteur",
    /**
     * ### CITIGROUP INC
     * *C.US/USD*
     */
    cususd = "cususd",
    /**
     * ### Carrefour SA
     * *CA.FR/EUR*
     */
    cafreur = "cafreur",
    /**
     * ### CaixaBank
     * *CABK.ES/EUR*
     */
    cabkeseur = "cabkeseur",
    /**
     * ### CREDIT ACCEPTANCE CORP
     * *CACC.US/USD*
     */
    caccususd = "caccususd",
    /**
     * ### CACI INTERNATIONAL INC -CL A
     * *CACI.US/USD*
     */
    caciususd = "caciususd",
    /**
     * ### Canadian Dollar vs Swiss Franc
     * *CAD/CHF*
     */
    cadchf = "cadchf",
    /**
     * ### Canadian Dollar vs Hong Kong Dollar
     * *CAD/HKD*
     */
    cadhkd = "cadhkd",
    /**
     * ### Canadian Dollar vs Japanese Yen
     * *CAD/JPY*
     */
    cadjpy = "cadjpy",
    /**
     * ### CONAGRA FOODS INC
     * *CAG.US/USD*
     */
    cagususd = "cagususd",
    /**
     * ### CARDINAL HEALTH INC
     * *CAH.US/USD*
     */
    cahususd = "cahususd",
    /**
     * ### Cap Gemini SA
     * *CAP.FR/EUR*
     */
    capfreur = "capfreur",
    /**
     * ### Carlsberg A/S
     * *CARLB.DK/DKK*
     */
    carlbdkdkk = "carlbdkdkk",
    /**
     * ### CARRIER GLOBAL CORP
     * *CARR.US/USD*
     */
    carrususd = "carrususd",
    /**
     * ### Cattolica Ass Coop A
     * *CASS.IT/EUR*
     */
    cassiteur = "cassiteur",
    /**
     * ### CASEY'S GENERAL STORES INC
     * *CASY.US/USD*
     */
    casyususd = "casyususd",
    /**
     * ### CATERPILLAR INC
     * *CAT.US/USD*
     */
    catususd = "catususd",
    /**
     * ### CHUBB LTD
     * *CB.US/USD*
     */
    cbususd = "cbususd",
    /**
     * ### Commerzbank AG
     * *CBK.DE/EUR*
     */
    cbkdeeur = "cbkdeeur",
    /**
     * ### CBOE GLOBAL MARKETS INC
     * *CBOE.US/USD*
     */
    cboeususd = "cboeususd",
    /**
     * ### CROWN CASTLE INTL CORP
     * *CCI.US/USD*
     */
    cciususd = "cciususd",
    /**
     * ### CROWN HOLDINGS INC
     * *CCK.US/USD*
     */
    cckususd = "cckususd",
    /**
     * ### Carnival PLC
     * *CCL.GB/GBX*
     */
    cclgbgbx = "cclgbgbx",
    /**
     * ### CARNIVAL CORP
     * *CCL.US/USD*
     */
    cclususd = "cclususd",
    /**
     * ### CERIDIAN HCM HOLDING INC
     * *CDAY.US/USD*
     */
    cdayususd = "cdayususd",
    /**
     * ### CENTENNIAL RESOURCE DEVELO-A
     * *CDEV.US/USD*
     */
    cdevususd = "cdevususd",
    /**
     * ### CDK GLOBAL INC
     * *CDK.US/USD*
     */
    cdkususd = "cdkususd",
    /**
     * ### CADENCE DESIGN SYS INC
     * *CDNS.US/USD*
     */
    cdnsususd = "cdnsususd",
    /**
     * ### CDW CORP/DE
     * *CDW.US/USD*
     */
    cdwususd = "cdwususd",
    /**
     * ### CELANESE CORP
     * *CE.US/USD*
     */
    ceususd = "ceususd",
    /**
     * ### Cemex Sab de CV
     * *CEMEXCPO.MX/MXN*
     */
    cemexcpomxmxn = "cemexcpomxmxn",
    /**
     * ### CERNER CORP
     * *CERN.US/USD*
     */
    cernususd = "cernususd",
    /**
     * ### Cerved Information Solutions SPA
     * *CERV.IT/EUR*
     */
    cerviteur = "cerviteur",
    /**
     * ### CF INDUSTRIES HOLDINGS INC
     * *CF.US/USD*
     */
    cfususd = "cfususd",
    /**
     * ### CITIZENS FINANCIAL GROUP
     * *CFG.US/USD*
     */
    cfgususd = "cfgususd",
    /**
     * ### Richemont
     * *CFR.CH/CHF*
     */
    cfrchchf = "cfrchchf",
    /**
     * ### CULLEN/FROST BANKERS INC
     * *CFR.US/USD*
     */
    cfrususd = "cfrususd",
    /**
     * ### COGNEX CORP
     * *CGNX.US/USD*
     */
    cgnxususd = "cgnxususd",
    /**
     * ### CHURCH & DWIGHT CO INC
     * *CHD.US/USD*
     */
    chdususd = "chdususd",
    /**
     * ### Swiss Franc vs Japanese Yen
     * *CHF/JPY*
     */
    chfjpy = "chfjpy",
    /**
     * ### Swiss Franc vs Singapore Dollar
     * *CHF/SGD*
     */
    chfsgd = "chfsgd",
    /**
     * ### China A50 Index
     * *CHI.IDX/USD*
     */
    chiidxusd = "chiidxusd",
    /**
     * ### CHANGE HEALTHCARE INC
     * *CHNG.US/USD*
     */
    chngususd = "chngususd",
    /**
     * ### CIGNA CORP
     * *CI.US/USD*
     */
    ciususd = "ciususd",
    /**
     * ### COLGATE-PALMOLIVE CO
     * *CL.US/USD*
     */
    clususd = "clususd",
    /**
     * ### Clariant AG
     * *CLN.CH/CHF*
     */
    clnchchf = "clnchchf",
    /**
     * ### CONTINENTAL RESOURCES INC/OK
     * *CLR.US/USD*
     */
    clrususd = "clrususd",
    /**
     * ### COMCAST CORP-CLASS A
     * *CMCSA.US/USD*
     */
    cmcsaususd = "cmcsaususd",
    /**
     * ### CME GROUP INC
     * *CME.US/USD*
     */
    cmeususd = "cmeususd",
    /**
     * ### CHIPOTLE MEXICAN GRILL INC
     * *CMG.US/USD*
     */
    cmgususd = "cmgususd",
    /**
     * ### CUMMINS INC
     * *CMI.US/USD*
     */
    cmiususd = "cmiususd",
    /**
     * ### Compound vs US Dollar
     * *CMP/USD*
     */
    cmpusd = "cmpusd",
    /**
     * ### Centrica PLC
     * *CNA.GB/GBX*
     */
    cnagbgbx = "cnagbgbx",
    /**
     * ### CENTENE CORP
     * *CNC.US/USD*
     */
    cncususd = "cncususd",
    /**
     * ### NY Cocoa
     * *COCOA.CMD/USD*
     */
    cocoacmdusd = "cocoacmdusd",
    /**
     * ### CAPITAL ONE FINANCIAL CORP
     * *COF.US/USD*
     */
    cofususd = "cofususd",
    /**
     * ### Coffee Arabica
     * *COFFEE.CMD/USX*
     */
    coffeecmdusx = "coffeecmdusx",
    /**
     * ### COHERENT INC
     * *COHR.US/USD*
     */
    cohrususd = "cohrususd",
    /**
     * ### AMERICOLD REALTY TRUST
     * *COLD.US/USD*
     */
    coldususd = "coldususd",
    /**
     * ### Coloplast A/S
     * *COLOB.DK/DKK*
     */
    colobdkdkk = "colobdkdkk",
    /**
     * ### Continental AG
     * *CON.DE/EUR*
     */
    condeeur = "condeeur",
    /**
     * ### COOPER COS INC/THE
     * *COO.US/USD*
     */
    cooususd = "cooususd",
    /**
     * ### CONOCOPHILLIPS
     * *COP.US/USD*
     */
    copususd = "copususd",
    /**
     * ### High Grade Copper
     * *COPPER.CMD/USD*
     */
    coppercmdusd = "coppercmdusd",
    /**
     * ### COSTCO WHOLESALE CORP
     * *COST.US/USD*
     */
    costususd = "costususd",
    /**
     * ### Cotton
     * *COTTON.CMD/USX*
     */
    cottoncmdusx = "cottoncmdusx",
    /**
     * ### COUPA SOFTWARE INC
     * *COUP.US/USD*
     */
    coupususd = "coupususd",
    /**
     * ### Covestro AG
     * *COV.DE/EUR*
     */
    covdeeur = "covdeeur",
    /**
     * ### Compass Group PLC
     * *CPG.GB/GBX*
     */
    cpggbgbx = "cpggbgbx",
    /**
     * ### Capita PLC
     * *CPI.GB/GBX*
     */
    cpigbgbx = "cpigbgbx",
    /**
     * ### Davide Campari-Milano Spa
     * *CPR.IT/EUR*
     */
    cpriteur = "cpriteur",
    /**
     * ### CAPRI HOLDINGS LTD
     * *CPRI.US/USD*
     */
    cpriususd = "cpriususd",
    /**
     * ### COPART INC
     * *CPRT.US/USD*
     */
    cprtususd = "cprtususd",
    /**
     * ### CAMDEN PROPERTY TRUST
     * *CPT.US/USD*
     */
    cptususd = "cptususd",
    /**
     * ### Croda International PLC
     * *CRDA.GB/GBX*
     */
    crdagbgbx = "crdagbgbx",
    /**
     * ### CRH Plc
     * *CRG.IE/EUR*
     */
    crgieeur = "crgieeur",
    /**
     * ### CRH PLC
     * *CRH.GB/GBX*
     */
    crhgbgbx = "crhgbgbx",
    /**
     * ### CHARLES RIVER LABORATORIES
     * *CRL.US/USD*
     */
    crlususd = "crlususd",
    /**
     * ### SALESFORCE.COM INC
     * *CRM.US/USD*
     */
    crmususd = "crmususd",
    /**
     * ### AXA SA
     * *CS.FR/EUR*
     */
    csfreur = "csfreur",
    /**
     * ### Credit Suisse Group AG
     * *CS.US/USD*
     */
    csususd = "csususd",
    /**
     * ### CISCO SYSTEMS INC
     * *CSCO.US/USD*
     */
    cscoususd = "cscoususd",
    /**
     * ### Credit Suisse Group AG
     * *CSGN.CH/CHF*
     */
    csgnchchf = "csgnchchf",
    /**
     * ### COSTAR GROUP INC
     * *CSGP.US/USD*
     */
    csgpususd = "csgpususd",
    /**
     * ### CARLISLE COS INC
     * *CSL.US/USD*
     */
    cslususd = "cslususd",
    /**
     * ### CSX CORP
     * *CSX.US/USD*
     */
    csxususd = "csxususd",
    /**
     * ### CENTURYLINK INC
     * *CTL.US/USD*
     */
    ctlususd = "ctlususd",
    /**
     * ### CATALENT INC
     * *CTLT.US/USD*
     */
    ctltususd = "ctltususd",
    /**
     * ### COGNIZANT TECH SOLUTIONS-A
     * *CTSH.US/USD*
     */
    ctshususd = "ctshususd",
    /**
     * ### CORTEVA INC
     * *CTVA.US/USD*
     */
    ctvaususd = "ctvaususd",
    /**
     * ### CITRIX SYSTEMS INC
     * *CTXS.US/USD*
     */
    ctxsususd = "ctxsususd",
    /**
     * ### CUBESMART
     * *CUBE.US/USD*
     */
    cubeususd = "cubeususd",
    /**
     * ### Credito Valtellinese Spa
     * *CVAL.IT/EUR*
     */
    cvaliteur = "cvaliteur",
    /**
     * ### CARVANA CO
     * *CVNA.US/USD*
     */
    cvnaususd = "cvnaususd",
    /**
     * ### CVS HEALTH CORP
     * *CVS.US/USD*
     */
    cvsususd = "cvsususd",
    /**
     * ### CHEVRON CORP
     * *CVX.US/USD*
     */
    cvxususd = "cvxususd",
    /**
     * ### CAESARS ENTERTAINMENT INC
     * *CZR.US/USD*
     */
    czrususd = "czrususd",
    /**
     * ### DOMINION RESOURCES INC/VA
     * *D.US/USD*
     */
    dususd = "dususd",
    /**
     * ### Daimler AG
     * *DAI.DE/EUR*
     */
    daideeur = "daideeur",
    /**
     * ### DELTA AIR LINES INC
     * *DAL.US/USD*
     */
    dalususd = "dalususd",
    /**
     * ### Danieli & Co Spa
     * *DAN.IT/EUR*
     */
    daniteur = "daniteur",
    /**
     * ### Danske Bank A/S
     * *DANSKE.DK/DKK*
     */
    danskedkdkk = "danskedkdkk",
    /**
     * ### Deutsche Boerse AG
     * *DB1.DE/EUR*
     */
    db1deeur = "db1deeur",
    /**
     * ### Deutsche Bank AG
     * *DBK.DE/EUR*
     */
    dbkdeeur = "dbkdeeur",
    /**
     * ### DROPBOX INC-CLASS A
     * *DBX.US/USD*
     */
    dbxususd = "dbxususd",
    /**
     * ### DCC PLC
     * *DCC.GB/GBX*
     */
    dccgbgbx = "dccgbgbx",
    /**
     * ### DEERE & CO
     * *DE.US/USD*
     */
    deususd = "deususd",
    /**
     * ### DELL TECHNOLOGIES -C
     * *DELL.US/USD*
     */
    dellususd = "dellususd",
    /**
     * ### DISCOVER FINANCIAL SERVICES
     * *DFS.US/USD*
     */
    dfsususd = "dfsususd",
    /**
     * ### Vinci SA
     * *DG.FR/EUR*
     */
    dgfreur = "dgfreur",
    /**
     * ### DOLLAR GENERAL CORP
     * *DG.US/USD*
     */
    dgususd = "dgususd",
    /**
     * ### Diageo PLC
     * *DGE.GB/GBX*
     */
    dgegbgbx = "dgegbgbx",
    /**
     * ### DR HORTON INC
     * *DHI.US/USD*
     */
    dhiususd = "dhiususd",
    /**
     * ### Delivery Hero AG
     * *DHR.DE/EUR*
     */
    dhrdeeur = "dhrdeeur",
    /**
     * ### DANAHER CORP
     * *DHR.US/USD*
     */
    dhrususd = "dhrususd",
    /**
     * ### Distribuidora Internacional de Alimentacion SA
     * *DIA.ES/EUR*
     */
    diaeseur = "diaeseur",
    /**
     * ### Diasorin Spa
     * *DIA.IT/EUR*
     */
    diaiteur = "diaiteur",
    /**
     * ### SPDR Dow Jones® Industrial Average ETF
     * *DIA.US/USD*
     */
    diaususd = "diaususd",
    /**
     * ### Gas oil
     * *DIESEL.CMD/USD*
     */
    dieselcmdusd = "dieselcmdusd",
    /**
     * ### WALT DISNEY CO/THE
     * *DIS.US/USD*
     */
    disususd = "disususd",
    /**
     * ### DICK'S SPORTING GOODS INC
     * *DKS.US/USD*
     */
    dksususd = "dksususd",
    /**
     * ### DIGITAL REALTY TRUST INC
     * *DLR.US/USD*
     */
    dlrususd = "dlrususd",
    /**
     * ### DOLLAR TREE INC
     * *DLTR.US/USD*
     */
    dltrususd = "dltrususd",
    /**
     * ### DNB ASA
     * *DNB.NO/NOK*
     */
    dnbnonok = "dnbnonok",
    /**
     * ### DOCUSIGN INC
     * *DOCU.US/USD*
     */
    docuususd = "docuususd",
    /**
     * ### US Dollar Index
     * *DOLLAR.IDX/USD*
     */
    dollaridxusd = "dollaridxusd",
    /**
     * ### AMDOCS LTD
     * *DOX.US/USD*
     */
    doxususd = "doxususd",
    /**
     * ### DECHRA PHARMACEUTICALS PLC
     * *DPH.GB/GBX*
     */
    dphgbgbx = "dphgbgbx",
    /**
     * ### Deutsche Post AG
     * *DPW.DE/EUR*
     */
    dpwdeeur = "dpwdeeur",
    /**
     * ### DOMINO'S PIZZA INC
     * *DPZ.US/USD*
     */
    dpzususd = "dpzususd",
    /**
     * ### DUKE REALTY CORP
     * *DRE.US/USD*
     */
    dreususd = "dreususd",
    /**
     * ### LYXOR ETF Daily X2 Short Bund
     * *DSB.FR/EUR*
     */
    dsbfreur = "dsbfreur",
    /**
     * ### Dashcoin vs US Dollar
     * *DSH/USD*
     */
    dshusd = "dshusd",
    /**
     * ### Koninklijke DSM NV
     * *DSM.NL/EUR*
     */
    dsmnleur = "dsmnleur",
    /**
     * ### DASSAULT SYSTEMES SE
     * *DSY.FR/EUR*
     */
    dsyfreur = "dsyfreur",
    /**
     * ### DYNATRACE INC
     * *DT.US/USD*
     */
    dtususd = "dtususd",
    /**
     * ### Deutsche Telekom AG
     * *DTE.DE/EUR*
     */
    dtedeeur = "dtedeeur",
    /**
     * ### Dukascoin vs Euro
     * *DUK+/EUR*
     */
    dukeur = "dukeur",
    /**
     * ### DUKE ENERGY CORP
     * *DUK.US/USD*
     */
    dukususd = "dukususd",
    /**
     * ### DAVITA HEALTHCARE PARTNERS I
     * *DVA.US/USD*
     */
    dvaususd = "dvaususd",
    /**
     * ### DEVON ENERGY CORP
     * *DVN.US/USD*
     */
    dvnususd = "dvnususd",
    /**
     * ### iShares Select Dividend ETF
     * *DVY.US/USD*
     */
    dvyususd = "dvyususd",
    /**
     * ### DXC TECHNOLOGY CO
     * *DXC.US/USD*
     */
    dxcususd = "dxcususd",
    /**
     * ### DEXCOM INC
     * *DXCM.US/USD*
     */
    dxcmususd = "dxcmususd",
    /**
     * ### ELECTRONIC ARTS INC
     * *EA.US/USD*
     */
    eaususd = "eaususd",
    /**
     * ### EBAY INC
     * *EBAY.US/USD*
     */
    ebayususd = "ebayususd",
    /**
     * ### Ebro Foods SA
     * *EBR.ES/EUR*
     */
    ebreseur = "ebreseur",
    /**
     * ### Erste Group Bank AG
     * *EBS.AT/EUR*
     */
    ebsateur = "ebsateur",
    /**
     * ### ELECTROCOMPONENTS PLC
     * *ECM.GB/GBX*
     */
    ecmgbgbx = "ecmgbgbx",
    /**
     * ### Electricite de France SA
     * *EDF.FR/EUR*
     */
    edffreur = "edffreur",
    /**
     * ### EDP - Energias de Portugal SA
     * *EDP.PT/EUR*
     */
    edppteur = "edppteur",
    /**
     * ### EURONET WORLDWIDE INC
     * *EEFT.US/USD*
     */
    eeftususd = "eeftususd",
    /**
     * ### iShares MSCI Emerging Markets ETF
     * *EEM.US/USD*
     */
    eemususd = "eemususd",
    /**
     * ### iShares MSCI EAFE ETF
     * *EFA.US/USD*
     */
    efaususd = "efaususd",
    /**
     * ### EQUIFAX INC
     * *EFX.US/USD*
     */
    efxususd = "efxususd",
    /**
     * ### ENCOMPASS HEALTH CORP
     * *EHC.US/USD*
     */
    ehcususd = "ehcususd",
    /**
     * ### EDISON INTERNATIONAL
     * *EIX.US/USD*
     */
    eixususd = "eixususd",
    /**
     * ### ESTEE LAUDER COMPANIES-CL A
     * *EL.US/USD*
     */
    elususd = "elususd",
    /**
     * ### ELANCO ANIMAL HEALTH INC
     * *ELAN.US/USD*
     */
    elanususd = "elanususd",
    /**
     * ### Endesa SA
     * *ELE.ES/EUR*
     */
    eleeseur = "eleeseur",
    /**
     * ### Grupo Elektra SAB DE CV
     * *ELEKTRA.MX/MXN*
     */
    elektramxmxn = "elektramxmxn",
    /**
     * ### Elisa OYJ
     * *ELI1V.FI/EUR*
     */
    eli1vfieur = "eli1vfieur",
    /**
     * ### EQUITY LIFESTYLE PROPERTIES
     * *ELS.US/USD*
     */
    elsususd = "elsususd",
    /**
     * ### Electrolux AB
     * *ELUXB.SE/SEK*
     */
    eluxbsesek = "eluxbsesek",
    /**
     * ### iShares J.P. Morgan USD Emerging Markets Bond ETF
     * *EMB.US/USD*
     */
    embususd = "embususd",
    /**
     * ### EMERSON ELECTRIC CO
     * *EMR.US/USD*
     */
    emrususd = "emrususd",
    /**
     * ### Bouygues SA
     * *EN.FR/EUR*
     */
    enfreur = "enfreur",
    /**
     * ### Enel SpA
     * *ENEL.IT/EUR*
     */
    eneliteur = "eneliteur",
    /**
     * ### Enagas SA
     * *ENG.ES/EUR*
     */
    engeseur = "engeseur",
    /**
     * ### Engie
     * *ENGI.FR/EUR*
     */
    engifreur = "engifreur",
    /**
     * ### ENI Spa
     * *ENI.IT/EUR*
     */
    eniiteur = "eniiteur",
    /**
     * ### Enjin vs US Dollar
     * *ENJ/USD*
     */
    enjusd = "enjusd",
    /**
     * ### ENPHASE ENERGY INC
     * *ENPH.US/USD*
     */
    enphususd = "enphususd",
    /**
     * ### ENTEGRIS INC
     * *ENTG.US/USD*
     */
    entgususd = "entgususd",
    /**
     * ### E.ON SE
     * *EOAN.DE/EUR*
     */
    eoandeeur = "eoandeeur",
    /**
     * ### EOG RESOURCES INC
     * *EOG.US/USD*
     */
    eogususd = "eogususd",
    /**
     * ### EOS vs US Dollar
     * *EOS/USD*
     */
    eosusd = "eosusd",
    /**
     * ### EPAM SYSTEMS INC
     * *EPAM.US/USD*
     */
    epamususd = "epamususd",
    /**
     * ### EQUITABLE HOLDINGS INC
     * *EQH.US/USD*
     */
    eqhususd = "eqhususd",
    /**
     * ### EQT CORP
     * *EQT.US/USD*
     */
    eqtususd = "eqtususd",
    /**
     * ### ERG Spa
     * *ERG.IT/EUR*
     */
    ergiteur = "ergiteur",
    /**
     * ### Telefonaktiebolaget LM Ericsson
     * *ERICB.SE/SEK*
     */
    ericbsesek = "ericbsesek",
    /**
     * ### EVERSOURCE ENERGY
     * *ES.US/USD*
     */
    esususd = "esususd",
    /**
     * ### ESSEX PROPERTY TRUST INC
     * *ESS.US/USD*
     */
    essususd = "essususd",
    /**
     * ### ELASTIC NV
     * *ESTC.US/USD*
     */
    estcususd = "estcususd",
    /**
     * ### Ether vs US Dollar
     * *ETH/USD*
     */
    ethusd = "ethusd",
    /**
     * ### EATON CORP PLC
     * *ETN.US/USD*
     */
    etnususd = "etnususd",
    /**
     * ### ETSY INC
     * *ETSY.US/USD*
     */
    etsyususd = "etsyususd",
    /**
     * ### Euro vs Australian Dollar
     * *EUR/AUD*
     */
    euraud = "euraud",
    /**
     * ### Euro vs Canadian Dollar
     * *EUR/CAD*
     */
    eurcad = "eurcad",
    /**
     * ### Euro vs Swiss Franc
     * *EUR/CHF*
     */
    eurchf = "eurchf",
    /**
     * ### Euro vs Czech Koruna
     * *EUR/CZK*
     */
    eurczk = "eurczk",
    /**
     * ### Euro vs Danish Krone
     * *EUR/DKK*
     */
    eurdkk = "eurdkk",
    /**
     * ### Euro vs Pound Sterling
     * *EUR/GBP*
     */
    eurgbp = "eurgbp",
    /**
     * ### Euro vs Hong Kong Dollar
     * *EUR/HKD*
     */
    eurhkd = "eurhkd",
    /**
     * ### Euro vs Hungarian Forint
     * *EUR/HUF*
     */
    eurhuf = "eurhuf",
    /**
     * ### Euro vs Japanese Yen
     * *EUR/JPY*
     */
    eurjpy = "eurjpy",
    /**
     * ### Euro vs Norwegian Krone
     * *EUR/NOK*
     */
    eurnok = "eurnok",
    /**
     * ### Euro vs New Zealand Dollar
     * *EUR/NZD*
     */
    eurnzd = "eurnzd",
    /**
     * ### Euro vs Polish Zloty
     * *EUR/PLN*
     */
    eurpln = "eurpln",
    /**
     * ### Euro vs Swedish Krona
     * *EUR/SEK*
     */
    eursek = "eursek",
    /**
     * ### Euro vs Singapore Dollar
     * *EUR/SGD*
     */
    eursgd = "eursgd",
    /**
     * ### Euro vs Turkish Lira
     * *EUR/TRY*
     */
    eurtry = "eurtry",
    /**
     * ### Euro vs US Dollar
     * *EUR/USD*
     */
    eurusd = "eurusd",
    /**
     * ### EVERGY INC
     * *EVRG.US/USD*
     */
    evrgususd = "evrgususd",
    /**
     * ### EDWARDS LIFESCIENCES CORP
     * *EW.US/USD*
     */
    ewususd = "ewususd",
    /**
     * ### EAST WEST BANCORP INC
     * *EWBC.US/USD*
     */
    ewbcususd = "ewbcususd",
    /**
     * ### iShares MSCI Hong Kong ETF
     * *EWH.US/USD*
     */
    ewhususd = "ewhususd",
    /**
     * ### iShares MSCI Japan ETF
     * *EWJ.US/USD*
     */
    ewjususd = "ewjususd",
    /**
     * ### iShares MSCI Mexico Capped
     * *EWW.US/USD*
     */
    ewwususd = "ewwususd",
    /**
     * ### iShares MSCI Brazil Capped
     * *EWZ.US/USD*
     */
    ewzususd = "ewzususd",
    /**
     * ### EXELON CORP
     * *EXC.US/USD*
     */
    excususd = "excususd",
    /**
     * ### EXELIXIS INC
     * *EXEL.US/USD*
     */
    exelususd = "exelususd",
    /**
     * ### EXPEDIA INC
     * *EXPE.US/USD*
     */
    expeususd = "expeususd",
    /**
     * ### Experian PLC
     * *EXPN.GB/GBX*
     */
    expngbgbx = "expngbgbx",
    /**
     * ### EXTRA SPACE STORAGE INC
     * *EXR.US/USD*
     */
    exrususd = "exrususd",
    /**
     * ### easyJet PLC
     * *EZJ.GB/GBX*
     */
    ezjgbgbx = "ezjgbgbx",
    /**
     * ### iShares MSCI EMU ETF
     * *EZU.US/USD*
     */
    ezuususd = "ezuususd",
    /**
     * ### US Brent Crude Oil
     * *BRENT.CMD/USD*
     */
    brentcmdusd = "brentcmdusd",
    /**
     * ### France 40 Index
     * *FRA.IDX/EUR*
     */
    fraidxeur = "fraidxeur",
    /**
     * ### USA 30 Index
     * *USA30.IDX/USD*
     */
    usa30idxusd = "usa30idxusd",
    /**
     * ### Germany 40 Index
     * *DEU.IDX/EUR*
     */
    deuidxeur = "deuidxeur",
    /**
     * ### Europe 50 Index
     * *EUS.IDX/EUR*
     */
    eusidxeur = "eusidxeur",
    /**
     * ### UK 100 Index
     * *GBR.IDX/GBP*
     */
    gbridxgbp = "gbridxgbp",
    /**
     * ### Hong Kong 40 Index
     * *HKG.IDX/HKD*
     */
    hkgidxhkd = "hkgidxhkd",
    /**
     * ### Spain 35 Index
     * *ESP.IDX/EUR*
     */
    espidxeur = "espidxeur",
    /**
     * ### US Light Crude Oil
     * *LIGHT.CMD/USD*
     */
    lightcmdusd = "lightcmdusd",
    /**
     * ### Japan 225
     * *JPN.IDX/JPY*
     */
    jpnidxjpy = "jpnidxjpy",
    /**
     * ### USA 100 Technical Index
     * *USATECH.IDX/USD*
     */
    usatechidxusd = "usatechidxusd",
    /**
     * ### USA 500 Index
     * *USA500.IDX/USD*
     */
    usa500idxusd = "usa500idxusd",
    /**
     * ### Switzerland 20 Index
     * *CHE.IDX/CHF*
     */
    cheidxchf = "cheidxchf",
    /**
     * ### Australia 200 Index
     * *AUS.IDX/AUD*
     */
    ausidxaud = "ausidxaud",
    /**
     * ### FORD MOTOR CO
     * *F.US/USD*
     */
    fususd = "fususd",
    /**
     * ### DIAMONDBACK ENERGY INC
     * *FANG.US/USD*
     */
    fangususd = "fangususd",
    /**
     * ### FACEBOOK INC-A
     * *FB.US/USD*
     */
    fbususd = "fbususd",
    /**
     * ### FORTUNE BRANDS HOME & SECURI
     * *FBHS.US/USD*
     */
    fbhsususd = "fbhsususd",
    /**
     * ### FinecoBank Banca Fineco Spa
     * *FBK.IT/EUR*
     */
    fbkiteur = "fbkiteur",
    /**
     * ### Fiat Chrysler Auto NV
     * *FCA.IT/EUR*
     */
    fcaiteur = "fcaiteur",
    /**
     * ### Fomento de Construcciones y Contratas SA
     * *FCC.ES/EUR*
     */
    fcceseur = "fcceseur",
    /**
     * ### FIRST CITIZENS BCSHS -CL A
     * *FCNCA.US/USD*
     */
    fcncaususd = "fcncaususd",
    /**
     * ### FREEPORT-MCMORAN INC
     * *FCX.US/USD*
     */
    fcxususd = "fcxususd",
    /**
     * ### FACTSET RESEARCH SYSTEMS INC
     * *FDS.US/USD*
     */
    fdsususd = "fdsususd",
    /**
     * ### FEDEX CORP
     * *FDX.US/USD*
     */
    fdxususd = "fdxususd",
    /**
     * ### FIRSTENERGY CORP
     * *FE.US/USD*
     */
    feususd = "feususd",
    /**
     * ### Fomento Economico Mexica UBD
     * *FEMSAUBD.MX/MXN*
     */
    femsaubdmxmxn = "femsaubdmxmxn",
    /**
     * ### Ferrovial SA
     * *FER.ES/EUR*
     */
    fereseur = "fereseur",
    /**
     * ### FAIR ISAAC CORP
     * *FICO.US/USD*
     */
    ficoususd = "ficoususd",
    /**
     * ### FIFTH THIRD BANCORP
     * *FITB.US/USD*
     */
    fitbususd = "fitbususd",
    /**
     * ### FIVE BELOW
     * *FIVE.US/USD*
     */
    fiveususd = "fiveususd",
    /**
     * ### FOOT LOCKER INC
     * *FL.US/USD*
     */
    flususd = "flususd",
    /**
     * ### FLEETCOR TECHNOLOGIES INC
     * *FLT.US/USD*
     */
    fltususd = "fltususd",
    /**
     * ### Fresenius Medical Care AG & Co KGaA
     * *FME.DE/EUR*
     */
    fmedeeur = "fmedeeur",
    /**
     * ### FLOOR & DECOR HOLDINGS INC-A
     * *FND.US/USD*
     */
    fndususd = "fndususd",
    /**
     * ### FIDELITY NATIONAL FINANCIAL
     * *FNF.US/USD*
     */
    fnfususd = "fnfususd",
    /**
     * ### TWENTY-FIRST CENTURY FOX-A
     * *FOXA.US/USD*
     */
    foxaususd = "foxaususd",
    /**
     * ### Total SA
     * *FP.FR/EUR*
     */
    fpfreur = "fpfreur",
    /**
     * ### Valeo SA
     * *FR.FR/EUR*
     */
    frfreur = "frfreur",
    /**
     * ### FIRST REPUBLIC BANK/CA
     * *FRC.US/USD*
     */
    frcususd = "frcususd",
    /**
     * ### Fresenius SE & Co KGaA
     * *FRE.DE/EUR*
     */
    fredeeur = "fredeeur",
    /**
     * ### Fresnillo PLC
     * *FRES.GB/GBX*
     */
    fresgbgbx = "fresgbgbx",
    /**
     * ### FMC TECHNOLOGIES INC
     * *FTI.US/USD*
     */
    ftiususd = "ftiususd",
    /**
     * ### FORTINET INC
     * *FTNT.US/USD*
     */
    ftntususd = "ftntususd",
    /**
     * ### FORTIVE CORP
     * *FTV.US/USD*
     */
    ftvususd = "ftvususd",
    /**
     * ### iShares China Large-Cap ETF
     * *FXI.US/USD*
     */
    fxiususd = "fxiususd",
    /**
     * ### Assicurazioni Generali Spa
     * *DLG.IT/EUR*
     */
    giteur = "giteur",
    /**
     * ### GENPACT LTD
     * *G.US/USD*
     */
    gususd = "gususd",
    /**
     * ### Galp Energia SGPS SA
     * *GALP.PT/EUR*
     */
    galppteur = "galppteur",
    /**
     * ### Gamesa Corporacion Tecnologica SA
     * *GAM.ES/EUR*
     */
    gameseur = "gameseur",
    /**
     * ### Grupo Aeroportuario del Pacific Sab CV
     * *GAPB.MX/MXN*
     */
    gapbmxmxn = "gapbmxmxn",
    /**
     * ### Natural Gas
     * *GAS.CMD/USD*
     */
    gascmdusd = "gascmdusd",
    /**
     * ### Gas Natural SDG SA
     * *GAS.ES/EUR*
     */
    gaseseur = "gaseseur",
    /**
     * ### Pound Sterling vs Australian Dollar
     * *GBP/AUD*
     */
    gbpaud = "gbpaud",
    /**
     * ### Pound Sterling vs Canadian Dollar
     * *GBP/CAD*
     */
    gbpcad = "gbpcad",
    /**
     * ### Pound Sterling vs Swiss Franc
     * *GBP/CHF*
     */
    gbpchf = "gbpchf",
    /**
     * ### Pound Sterling vs Japanese Yen
     * *GBP/JPY*
     */
    gbpjpy = "gbpjpy",
    /**
     * ### Pound Sterling vs New Zealand Dollar
     * *GBP/NZD*
     */
    gbpnzd = "gbpnzd",
    /**
     * ### Pound Sterling vs US Dollar
     * *GBP/USD*
     */
    gbpusd = "gbpusd",
    /**
     * ### Grupo Carso Sab De CV
     * *GCARSOA1.MX/MXN*
     */
    gcarsoa1mxmxn = "gcarsoa1mxmxn",
    /**
     * ### Grupo Cementos de Chihuahua
     * *GCC.MX/MXN*
     */
    gccmxmxn = "gccmxmxn",
    /**
     * ### GENERAL DYNAMICS CORP
     * *GD.US/USD*
     */
    gdususd = "gdususd",
    /**
     * ### GODADDY INC - CLASS A
     * *GDDY.US/USD*
     */
    gddyususd = "gddyususd",
    /**
     * ### VanEck Vectors Gold Miners ETF
     * *GDX.US/USD*
     */
    gdxususd = "gdxususd",
    /**
     * ### VanEck Vectors Junior Gold Miners ETF
     * *GDXJ.US/USD*
     */
    gdxjususd = "gdxjususd",
    /**
     * ### GENERAL ELECTRIC CO
     * *GE.US/USD*
     */
    geususd = "geususd",
    /**
     * ### Getinge AB
     * *GETIB.SE/SEK*
     */
    getibsesek = "getibsesek",
    /**
     * ### Grupo Financiero Banorte O
     * *GFNORTEO.MX/MXN*
     */
    gfnorteomxmxn = "gfnorteomxmxn",
    /**
     * ### G4S PLC
     * *GFS.GB/GBX*
     */
    gfsgbgbx = "gfsgbgbx",
    /**
     * ### GRACO INC
     * *GGG.US/USD*
     */
    gggususd = "gggususd",
    /**
     * ### GUARDANT HEALTH INC
     * *GH.US/USD*
     */
    ghususd = "ghususd",
    /**
     * ### GILEAD SCIENCES INC
     * *GILD.US/USD*
     */
    gildususd = "gildususd",
    /**
     * ### GENERAL MILLS INC
     * *GIS.US/USD*
     */
    gisususd = "gisususd",
    /**
     * ### Givaudan SA
     * *GIVN.CH/CHF*
     */
    givnchchf = "givnchchf",
    /**
     * ### GLOBE LIFE INC
     * *GL.US/USD*
     */
    glususd = "glususd",
    /**
     * ### SPDR Gold Shares ETF
     * *GLD.US/USD*
     */
    gldususd = "gldususd",
    /**
     * ### Societe Generale SA
     * *GLE.FR/EUR*
     */
    glefreur = "glefreur",
    /**
     * ### Glencore PLC
     * *GLEN.GB/GBX*
     */
    glengbgbx = "glengbgbx",
    /**
     * ### GAMING AND LEISURE PROPERTIE
     * *GLPI.US/USD*
     */
    glpiususd = "glpiususd",
    /**
     * ### CORNING INC
     * *GLW.US/USD*
     */
    glwususd = "glwususd",
    /**
     * ### GENERAL MOTORS CO
     * *GM.US/USD*
     */
    gmususd = "gmususd",
    /**
     * ### Grupo Mexico SAB DE CV SER B
     * *GMEXICOB.MX/MXN*
     */
    gmexicobmxmxn = "gmexicobmxmxn",
    /**
     * ### GENERAC HOLDINGS INC
     * *GNRC.US/USD*
     */
    gnrcususd = "gnrcususd",
    /**
     * ### ALPHABET INC-CL C
     * *GOOG.US/USD*
     */
    googususd = "googususd",
    /**
     * ### ALPHABET INC-CL A
     * *GOOGL.US/USD*
     */
    googlususd = "googlususd",
    /**
     * ### GLOBAL PAYMENTS INC
     * *GPN.US/USD*
     */
    gpnususd = "gpnususd",
    /**
     * ### GAP INC/THE
     * *GPS.US/USD*
     */
    gpsususd = "gpsususd",
    /**
     * ### GRIFOLS SA
     * *GRF.ES/EUR*
     */
    grfeseur = "grfeseur",
    /**
     * ### Gruma SA BB
     * *GRUMAB.MX/MXN*
     */
    grumabmxmxn = "grumabmxmxn",
    /**
     * ### GOLDMAN SACHS GROUP INC
     * *GS.US/USD*
     */
    gsususd = "gsususd",
    /**
     * ### GlaxoSmithKline PLC
     * *GSK.GB/GBX*
     */
    gskgbgbx = "gskgbgbx",
    /**
     * ### WW GRAINGER INC
     * *GWW.US/USD*
     */
    gwwususd = "gwwususd",
    /**
     * ### HYATT HOTELS CORP - CL A
     * *H.US/USD*
     */
    hususd = "hususd",
    /**
     * ### HALLIBURTON CO
     * *HAL.US/USD*
     */
    halususd = "halususd",
    /**
     * ### HANESBRANDS INC
     * *HBI.US/USD*
     */
    hbiususd = "hbiususd",
    /**
     * ### HCA HEALTHCARE INC
     * *HCA.US/USD*
     */
    hcaususd = "hcaususd",
    /**
     * ### WELLTOWER INC
     * *HCN.US/USD*
     */
    hcnususd = "hcnususd",
    /**
     * ### HOME DEPOT INC
     * *HD.US/USD*
     */
    hdususd = "hdususd",
    /**
     * ### HeidelbergCement AG
     * *HEI.DE/EUR*
     */
    heideeur = "heideeur",
    /**
     * ### HEICO CORP
     * *HEI.US/USD*
     */
    heiususd = "heiususd",
    /**
     * ### Heineken NV
     * *HEIA.NL/EUR*
     */
    heianleur = "heianleur",
    /**
     * ### Henkel AG & Co KGaA
     * *HEN3.DE/EUR*
     */
    hen3deeur = "hen3deeur",
    /**
     * ### HESS CORP
     * *HES.US/USD*
     */
    hesususd = "hesususd",
    /**
     * ### HELLOFRESH SE
     * *HFG.DE/EUR*
     */
    hfgdeeur = "hfgdeeur",
    /**
     * ### HUNTINGTON INGALLS INDUSTRIE
     * *HII.US/USD*
     */
    hiiususd = "hiiususd",
    /**
     * ### HIKMA PHARMACEUTICALS PLC
     * *HIK.GB/GBX*
     */
    hikgbgbx = "hikgbgbx",
    /**
     * ### Hong Kong Dollar vs Japanese Yen
     * *HKD/JPY*
     */
    hkdjpy = "hkdjpy",
    /**
     * ### HALMA PLC
     * *HLMA.GB/GBX*
     */
    hlmagbgbx = "hlmagbgbx",
    /**
     * ### HILTON WORLDWIDE HOLDINGS IN
     * *HLT.US/USD*
     */
    hltususd = "hltususd",
    /**
     * ### Hennes & Mauritz AB
     * *HMB.SE/SEK*
     */
    hmbsesek = "hmbsesek",
    /**
     * ### Hammerson PLC
     * *HMSO.GB/GBX*
     */
    hmsogbgbx = "hmsogbgbx",
    /**
     * ### Hannover Rueckversicheru-Reg
     * *HNR.DE/EUR*
     */
    hnrdeeur = "hnrdeeur",
    /**
     * ### THALES SA
     * *HO.FR/EUR*
     */
    hofreur = "hofreur",
    /**
     * ### HARLEY-DAVIDSON INC
     * *HOG.US/USD*
     */
    hogususd = "hogususd",
    /**
     * ### HONEYWELL INTERNATIONAL INC
     * *HON.US/USD*
     */
    honususd = "honususd",
    /**
     * ### HEWLETT PACKARD ENTERPRISE
     * *HPE.US/USD*
     */
    hpeususd = "hpeususd",
    /**
     * ### HP INC
     * *HPQ.US/USD*
     */
    hpqususd = "hpqususd",
    /**
     * ### HSBC Holdings PLC
     * *HSBA.GB/GBX*
     */
    hsbagbgbx = "hsbagbgbx",
    /**
     * ### HEALTHCARE TRUST OF AME-CL A
     * *HTA.US/USD*
     */
    htaususd = "htaususd",
    /**
     * ### HUBBELL INC
     * *HUBB.US/USD*
     */
    hubbususd = "hubbususd",
    /**
     * ### HUBSPOT INC
     * *HUBS.US/USD*
     */
    hubsususd = "hubsususd",
    /**
     * ### HUMANA INC
     * *HUM.US/USD*
     */
    humususd = "humususd",
    /**
     * ### HOWMET AEROSPACE INC
     * *HWM.US/USD*
     */
    hwmususd = "hwmususd",
    /**
     * ### HORIZON THERAPEUTICS PLC
     * *HZNP.US/USD*
     */
    hznpususd = "hznpususd",
    /**
     * ### International Consolidated Airlines Grou
     * *IAG.GB/GBX*
     */
    iaggbgbx = "iaggbgbx",
    /**
     * ### iShares Nasdaq Biotechnology ETF
     * *IBB.US/USD*
     */
    ibbususd = "ibbususd",
    /**
     * ### Iberdrola SA
     * *IBE.ES/EUR*
     */
    ibeeseur = "ibeeseur",
    /**
     * ### INTERACTIVE BROKERS GRO-CL A
     * *IBKR.US/USD*
     */
    ibkrususd = "ibkrususd",
    /**
     * ### INTL BUSINESS MACHINES CORP
     * *IBM.US/USD*
     */
    ibmususd = "ibmususd",
    /**
     * ### INTERCONTINENTAL EXCHANGE IN
     * *ICE.US/USD*
     */
    iceususd = "iceususd",
    /**
     * ### INTERMEDIATE CAPITAL GROUP
     * *ICP.GB/GBX*
     */
    icpgbgbx = "icpgbgbx",
    /**
     * ### IDEXX LABORATORIES INC
     * *IDXX.US/USD*
     */
    idxxususd = "idxxususd",
    /**
     * ### iShares 7-10 Year Treasury Bond ETF
     * *IEF.US/USD*
     */
    iefususd = "iefususd",
    /**
     * ### IDEX CORP
     * *IEX.US/USD*
     */
    iexususd = "iexususd",
    /**
     * ### Infineon Technologies AG
     * *IFX.DE/EUR*
     */
    ifxdeeur = "ifxdeeur",
    /**
     * ### Italgas Spa
     * *IG.IT/EUR*
     */
    igiteur = "igiteur",
    /**
     * ### InterContinental Hotels Group PLC
     * *IHG.GB/GBX*
     */
    ihggbgbx = "ihggbgbx",
    /**
     * ### 3i Group PLC
     * *III.GB/GBX*
     */
    iiigbgbx = "iiigbgbx",
    /**
     * ### iShares Core S&P Mid-Cap ETF
     * *IJH.US/USD*
     */
    ijhususd = "ijhususd",
    /**
     * ### iShares Core S&P Small-Cap ETF
     * *IJR.US/USD*
     */
    ijrususd = "ijrususd",
    /**
     * ### ILLUMINA INC
     * *ILMN.US/USD*
     */
    ilmnususd = "ilmnususd",
    /**
     * ### Imperial Brands Plc
     * *IMT.GB/GBX*
     */
    imtgbgbx = "imtgbgbx",
    /**
     * ### INCYTE CORP
     * *INCY.US/USD*
     */
    incyususd = "incyususd",
    /**
     * ### India 50 Index
     * *IND.IDX/USD*
     */
    indidxusd = "indidxusd",
    /**
     * ### ING Groep NV
     * *INGA.NL/EUR*
     */
    inganleur = "inganleur",
    /**
     * ### INTEL CORP
     * *INTC.US/USD*
     */
    intcususd = "intcususd",
    /**
     * ### INTUIT INC
     * *INTU.US/USD*
     */
    intuususd = "intuususd",
    /**
     * ### Investor AB
     * *INVEB.SE/SEK*
     */
    invebsesek = "invebsesek",
    /**
     * ### INVITATION HOMES INC
     * *INVH.US/USD*
     */
    invhususd = "invhususd",
    /**
     * ### Infrastrutture Wireless Italiane Spa
     * *INW.IT/EUR*
     */
    inwiteur = "inwiteur",
    /**
     * ### IONIS PHARMACEUTICALS INC
     * *IONS.US/USD*
     */
    ionsususd = "ionsususd",
    /**
     * ### INTERPUBLIC GROUP OF COS INC
     * *IPG.US/USD*
     */
    ipgususd = "ipgususd",
    /**
     * ### IPG PHOTONICS CORP
     * *IPGP.US/USD*
     */
    ipgpususd = "ipgpususd",
    /**
     * ### IQVIA HOLDINGS INC
     * *IQV.US/USD*
     */
    iqvususd = "iqvususd",
    /**
     * ### INGERSOLL-RAND INC
     * *IR.US/USD*
     */
    irususd = "irususd",
    /**
     * ### Intesa Sanpaolo Spa
     * *ISP.IT/EUR*
     */
    ispiteur = "ispiteur",
    /**
     * ### INTUITIVE SURGICAL INC
     * *ISRG.US/USD*
     */
    isrgususd = "isrgususd",
    /**
     * ### GARTNER INC
     * *IT.US/USD*
     */
    itususd = "itususd",
    /**
     * ### Intertek Group PLC
     * *ITRK.GB/GBX*
     */
    itrkgbgbx = "itrkgbgbx",
    /**
     * ### Itau Unibanco Holding S.A.
     * *ITUB.US/USD*
     */
    itubususd = "itubususd",
    /**
     * ### ITV PLC
     * *ITV.GB/GBX*
     */
    itvgbgbx = "itvgbgbx",
    /**
     * ### ILLINOIS TOOL WORKS
     * *ITW.US/USD*
     */
    itwususd = "itwususd",
    /**
     * ### Inditex SA
     * *ITX.ES/EUR*
     */
    itxeseur = "itxeseur",
    /**
     * ### iShares S&P 500 Value ETF
     * *IVE.US/USD*
     */
    iveususd = "iveususd",
    /**
     * ### iShares S&P 500 Growth ETF
     * *IVW.US/USD*
     */
    ivwususd = "ivwususd",
    /**
     * ### INVESCO LTD
     * *IVZ.US/USD*
     */
    ivzususd = "ivzususd",
    /**
     * ### iShares Russell 1000 Value ETF
     * *IWD.US/USD*
     */
    iwdususd = "iwdususd",
    /**
     * ### iShares Russell 1000 Growth ETF
     * *IWF.US/USD*
     */
    iwfususd = "iwfususd",
    /**
     * ### iShares Russell 2000 ETF
     * *IWM.US/USD*
     */
    iwmususd = "iwmususd",
    /**
     * ### iShares U.S. Real Estate ETF
     * *IYR.US/USD*
     */
    iyrususd = "iyrususd",
    /**
     * ### JAZZ PHARMACEUTICALS PLC
     * *JAZZ.US/USD*
     */
    jazzususd = "jazzususd",
    /**
     * ### JETBLUE AIRWAYS CORP
     * *JBLU.US/USD*
     */
    jbluususd = "jbluususd",
    /**
     * ### JOHNSON CONTROLS INC
     * *JCI.US/USD*
     */
    jciususd = "jciususd",
    /**
     * ### JEFFERIES FINANCIAL GROUP IN
     * *JEF.US/USD*
     */
    jefususd = "jefususd",
    /**
     * ### JACK HENRY & ASSOCIATES INC
     * *JKHY.US/USD*
     */
    jkhyususd = "jkhyususd",
    /**
     * ### JONES LANG LASALLE INC
     * *JLL.US/USD*
     */
    jllususd = "jllususd",
    /**
     * ### JOHNSON & JOHNSON
     * *JNJ.US/USD*
     */
    jnjususd = "jnjususd",
    /**
     * ### SPDR Barclays Capital High Yield Bond ETF
     * *JNK.US/USD*
     */
    jnkususd = "jnkususd",
    /**
     * ### JPMORGAN CHASE & CO
     * *JPM.US/USD*
     */
    jpmususd = "jpmususd",
    /**
     * ### Juventus Football Club Spa
     * *JUVE.IT/EUR*
     */
    juveiteur = "juveiteur",
    /**
     * ### NORDSTROM INC
     * *JWN.US/USD*
     */
    jwnususd = "jwnususd",
    /**
     * ### KELLOGG CO
     * *K.US/USD*
     */
    kususd = "kususd",
    /**
     * ### KBC Groep NV
     * *KBC.BE/EUR*
     */
    kbcbeeur = "kbcbeeur",
    /**
     * ### Kering
     * *KER.FR/EUR*
     */
    kerfreur = "kerfreur",
    /**
     * ### KEYCORP
     * *KEY.US/USD*
     */
    keyususd = "keyususd",
    /**
     * ### KEYSIGHT TECHNOLOGIES IN
     * *KEYS.US/USD*
     */
    keysususd = "keysususd",
    /**
     * ### Kingfisher PLC
     * *KGF.GB/GBX*
     */
    kgfgbgbx = "kgfgbgbx",
    /**
     * ### KRAFT HEINZ CO/THE
     * *KHC.US/USD*
     */
    khcususd = "khcususd",
    /**
     * ### Kimberly Clark DE Mexico A
     * *KIMBERA.MX/MXN*
     */
    kimberamxmxn = "kimberamxmxn",
    /**
     * ### KIMBERLY-CLARK CORP
     * *KMB.US/USD*
     */
    kmbususd = "kmbususd",
    /**
     * ### KINDER MORGAN INC
     * *KMI.US/USD*
     */
    kmiususd = "kmiususd",
    /**
     * ### CARMAX INC
     * *KMX.US/USD*
     */
    kmxususd = "kmxususd",
    /**
     * ### Kuehne + Nagel International AG
     * *KNIN.CH/CHF*
     */
    kninchchf = "kninchchf",
    /**
     * ### KNIGHT-SWIFT TRANSPORTATION
     * *KNX.US/USD*
     */
    knxususd = "knxususd",
    /**
     * ### COCA-COLA CO/THE
     * *KO.US/USD*
     */
    koususd = "koususd",
    /**
     * ### Coca Cola Femsa Sab
     * *KOFUBL.MX/MXN*
     */
    kofublmxmxn = "kofublmxmxn",
    /**
     * ### Koninklijke KPN NV
     * *KPN.NL/EUR*
     */
    kpnnleur = "kpnnleur",
    /**
     * ### KROGER CO
     * *KR.US/USD*
     */
    krususd = "krususd",
    /**
     * ### KILROY REALTY CORP
     * *KRC.US/USD*
     */
    krcususd = "krcususd",
    /**
     * ### Kingspan Group Plc
     * *KRX.IE/EUR*
     */
    krxieeur = "krxieeur",
    /**
     * ### Kerry Group Plc
     * *KRZ.IE/EUR*
     */
    krzieeur = "krzieeur",
    /**
     * ### KOHLS CORP
     * *KSS.US/USD*
     */
    kssususd = "kssususd",
    /**
     * ### LOEWS CORP
     * *L.US/USD*
     */
    lususd = "lususd",
    /**
     * ### Genomma Lab Internacional SAB de CV
     * *LABB.MX/MXN*
     */
    labbmxmxn = "labbmxmxn",
    /**
     * ### LAMAR ADVERTISING CO-A
     * *LAMR.US/USD*
     */
    lamrususd = "lamrususd",
    /**
     * ### Land Securities Group PLC
     * *LAND.GB/GBX*
     */
    landgbgbx = "landgbgbx",
    /**
     * ### Leonardo Spa
     * *LDO.IT/EUR*
     */
    ldoiteur = "ldoiteur",
    /**
     * ### LEIDOS HOLDINGS INC
     * *LDOS.US/USD*
     */
    ldosususd = "ldosususd",
    /**
     * ### LEAR CORP
     * *LEA.US/USD*
     */
    leaususd = "leaususd",
    /**
     * ### LENNAR CORP-A
     * *LEN.US/USD*
     */
    lenususd = "lenususd",
    /**
     * ### Legal & General Group PLC
     * *LGEN.GB/GBX*
     */
    lgengbgbx = "lgengbgbx",
    /**
     * ### Deutsche Lufthansa AG
     * *LHA.DE/EUR*
     */
    lhadeeur = "lhadeeur",
    /**
     * ### Lafarge Holcim Ltd
     * *LHN.CH/CHF*
     */
    lhnchchf = "lhnchchf",
    /**
     * ### L3HARRIS TECHNOLOGIES INC
     * *LHX.US/USD*
     */
    lhxususd = "lhxususd",
    /**
     * ### Klepierre
     * *LI.FR/EUR*
     */
    lifreur = "lifreur",
    /**
     * ### Linde AG
     * *LIN.DE/EUR*
     */
    lindeeur = "lindeeur",
    /**
     * ### Chocoladefabriken Lindt & Sprungli
     * *LISP.CH/CHF*
     */
    lispchchf = "lispchchf",
    /**
     * ### El Puerto de Liverpool
     * *LIVEPOLC1.MX/MXN*
     */
    livepolc1mxmxn = "livepolc1mxmxn",
    /**
     * ### LKQ CORP
     * *LKQ.US/USD*
     */
    lkqususd = "lkqususd",
    /**
     * ### Lloyds Banking Group PLC
     * *LLOY.GB/GBX*
     */
    lloygbgbx = "lloygbgbx",
    /**
     * ### ELI LILLY & CO
     * *LLY.US/USD*
     */
    llyususd = "llyususd",
    /**
     * ### LOCKHEED MARTIN CORP
     * *LMT.US/USD*
     */
    lmtususd = "lmtususd",
    /**
     * ### LINCOLN NATIONAL CORP
     * *LNC.US/USD*
     */
    lncususd = "lncususd",
    /**
     * ### CHENIERE ENERGY INC
     * *LNG.US/USD*
     */
    lngususd = "lngususd",
    /**
     * ### Chainlink vs US Dollar
     * *LNK/USD*
     */
    lnkusd = "lnkusd",
    /**
     * ### ALLIANT ENERGY CORP
     * *LNT.US/USD*
     */
    lntususd = "lntususd",
    /**
     * ### Logitech International SA
     * *LOGN.CH/CHF*
     */
    lognchchf = "lognchchf",
    /**
     * ### Lonza Group AG
     * *LONN.CH/CHF*
     */
    lonnchchf = "lonnchchf",
    /**
     * ### LOWE'S COS INC
     * *LOW.US/USD*
     */
    lowususd = "lowususd",
    /**
     * ### LPL FINANCIAL HOLDINGS INC
     * *LPLA.US/USD*
     */
    lplaususd = "lplaususd",
    /**
     * ### Legrand SA
     * *LR.FR/EUR*
     */
    lrfreur = "lrfreur",
    /**
     * ### LAM RESEARCH CORP
     * *LRCX.US/USD*
     */
    lrcxususd = "lrcxususd",
    /**
     * ### London Stock Exchange Group PLC
     * *LSE.GB/GBX*
     */
    lsegbgbx = "lsegbgbx",
    /**
     * ### LIFE STORAGE INC
     * *LSI.US/USD*
     */
    lsiususd = "lsiususd",
    /**
     * ### LANDSTAR SYSTEM INC
     * *LSTR.US/USD*
     */
    lstrususd = "lstrususd",
    /**
     * ### Litecoin vs US Dollar
     * *LTC/USD*
     */
    ltcusd = "ltcusd",
    /**
     * ### LULULEMON ATHLETICA INC
     * *LULU.US/USD*
     */
    luluususd = "luluususd",
    /**
     * ### SOUTHWEST AIRLINES CO
     * *LUV.US/USD*
     */
    luvususd = "luvususd",
    /**
     * ### Lyxor ETF Leverage CAC 40
     * *LVC.FR/EUR*
     */
    lvcfreur = "lvcfreur",
    /**
     * ### Las Vegas Sands Corp.
     * *LVS.US/USD*
     */
    lvsususd = "lvsususd",
    /**
     * ### LAMB WESTON HOLDINGS INC
     * *LW.US/USD*
     */
    lwususd = "lwususd",
    /**
     * ### Lanxess AG
     * *LXS.DE/EUR*
     */
    lxsdeeur = "lxsdeeur",
    /**
     * ### LYONDELLBASELL INDU-CL A
     * *LYB.US/USD*
     */
    lybususd = "lybususd",
    /**
     * ### LYFT INC-A
     * *LYFT.US/USD*
     */
    lyftususd = "lyftususd",
    /**
     * ### LIVE NATION ENTERTAINMENT IN
     * *LYV.US/USD*
     */
    lyvususd = "lyvususd",
    /**
     * ### Lyxor UCITS ETF STOXX EUROPE 600 Banks
     * *LYXBNK.FR/EUR*
     */
    lyxbnkfreur = "lyxbnkfreur",
    /**
     * ### MACY'S INC
     * *M.US/USD*
     */
    mususd = "mususd",
    /**
     * ### MASTERCARD INC-CLASS A
     * *MA.US/USD*
     */
    maususd = "maususd",
    /**
     * ### MID-AMERICA APARTMENT COMM
     * *MAA.US/USD*
     */
    maaususd = "maaususd",
    /**
     * ### AP Moeller - Maersk A/S
     * *MAERSKB.DK/DKK*
     */
    maerskbdkdkk = "maerskbdkdkk",
    /**
     * ### MANHATTAN ASSOCIATES INC
     * *MANH.US/USD*
     */
    manhususd = "manhususd",
    /**
     * ### Mapfre SA
     * *MAP.ES/EUR*
     */
    mapeseur = "mapeseur",
    /**
     * ### MARRIOTT INTERNATIONAL -CL A
     * *MAR.US/USD*
     */
    marususd = "marususd",
    /**
     * ### MASIMO CORP
     * *MASI.US/USD*
     */
    masiususd = "masiususd",
    /**
     * ### MATTEL INC
     * *MAT.US/USD*
     */
    matususd = "matususd",
    /**
     * ### Polygon vs US Dollar
     * *MAT/USD*
     */
    matusd = "matusd",
    /**
     * ### Mediobanca Spa
     * *MB.IT/EUR*
     */
    mbiteur = "mbiteur",
    /**
     * ### Meyer Burger
     * *MBTN.CH/CHF*
     */
    mbtnchchf = "mbtnchchf",
    /**
     * ### LVMH Moet Hennessy Louis Vuitton SA
     * *MC.FR/EUR*
     */
    mcfreur = "mcfreur",
    /**
     * ### MCDONALD'S CORP
     * *MCD.US/USD*
     */
    mcdususd = "mcdususd",
    /**
     * ### MICROCHIP TECHNOLOGY INC
     * *MCHP.US/USD*
     */
    mchpususd = "mchpususd",
    /**
     * ### MCKESSON CORP
     * *MCK.US/USD*
     */
    mckususd = "mckususd",
    /**
     * ### MONGODB INC
     * *MDB.US/USD*
     */
    mdbususd = "mdbususd",
    /**
     * ### MONDELEZ INTERNATIONAL INC-A
     * *MDLZ.US/USD*
     */
    mdlzususd = "mdlzususd",
    /**
     * ### MEDTRONIC PLC
     * *MDT.US/USD*
     */
    mdtususd = "mdtususd",
    /**
     * ### Megacable Holdings SAB de CV
     * *MEGACPO.MX/MXN*
     */
    megacpomxmxn = "megacpomxmxn",
    /**
     * ### METLIFE INC
     * *MET.US/USD*
     */
    metususd = "metususd",
    /**
     * ### Meggitt PLC
     * *MGGT.GB/GBX*
     */
    mggtgbgbx = "mggtgbgbx",
    /**
     * ### MGM RESORTS INTERNATIONAL
     * *MGM.US/USD*
     */
    mgmususd = "mgmususd",
    /**
     * ### Marine Harvest ASA
     * *MHG.NO/NOK*
     */
    mhgnonok = "mhgnonok",
    /**
     * ### MOHAWK INDUSTRIES INC
     * *MHK.US/USD*
     */
    mhkususd = "mhkususd",
    /**
     * ### MIDDLEBY CORP
     * *MIDD.US/USD*
     */
    middususd = "middususd",
    /**
     * ### MARKEL CORP
     * *MKL.US/USD*
     */
    mklususd = "mklususd",
    /**
     * ### Maker vs US Dollar
     * *MKR/USD*
     */
    mkrusd = "mkrusd",
    /**
     * ### Marks & Spencer Group PLC
     * *MKS.GB/GBX*
     */
    mksgbgbx = "mksgbgbx",
    /**
     * ### MKS INSTRUMENTS INC
     * *MKSI.US/USD*
     */
    mksiususd = "mksiususd",
    /**
     * ### MARKETAXESS HOLDINGS INC
     * *MKTX.US/USD*
     */
    mktxususd = "mktxususd",
    /**
     * ### Michelin SA
     * *ML.FR/EUR*
     */
    mlfreur = "mlfreur",
    /**
     * ### MARTIN MARIETTA MATERIALS
     * *MLM.US/USD*
     */
    mlmususd = "mlmususd",
    /**
     * ### 3M CO
     * *MMM.US/USD*
     */
    mmmususd = "mmmususd",
    /**
     * ### Mondi PLC
     * *MNDI.GB/GBX*
     */
    mndigbgbx = "mndigbgbx",
    /**
     * ### MANDIANT INC
     * *MNDT.US/USD*
     */
    mndtususd = "mndtususd",
    /**
     * ### M&G PLC
     * *MNG.GB/GBX*
     */
    mnggbgbx = "mnggbgbx",
    /**
     * ### ALTRIA GROUP INC
     * *MO.US/USD*
     */
    moususd = "moususd",
    /**
     * ### MOLINA HEALTHCARE INC
     * *MOH.US/USD*
     */
    mohususd = "mohususd",
    /**
     * ### Moncler Spa
     * *MONC.IT/EUR*
     */
    monciteur = "monciteur",
    /**
     * ### MARATHON PETROLEUM CORP
     * *MPC.US/USD*
     */
    mpcususd = "mpcususd",
    /**
     * ### MEDICAL PROPERTIES TRUST INC
     * *MPW.US/USD*
     */
    mpwususd = "mpwususd",
    /**
     * ### MONOLITHIC POWER SYSTEMS INC
     * *MPWR.US/USD*
     */
    mpwrususd = "mpwrususd",
    /**
     * ### Merck KGaA
     * *MRK.DE/EUR*
     */
    mrkdeeur = "mrkdeeur",
    /**
     * ### MERCK & CO. INC.
     * *MRK.US/USD*
     */
    mrkususd = "mrkususd",
    /**
     * ### Moderna
     * *MRNA.US/USD*
     */
    mrnaususd = "mrnaususd",
    /**
     * ### MARATHON OIL CORP
     * *MRO.US/USD*
     */
    mroususd = "mroususd",
    /**
     * ### MARVELL TECHNOLOGY INC
     * *MRVL.US/USD*
     */
    mrvlususd = "mrvlususd",
    /**
     * ### WM Morrison Supermarkets PLC
     * *MRW.GB/GBX*
     */
    mrwgbgbx = "mrwgbgbx",
    /**
     * ### Mediaset Spa
     * *MS.IT/EUR*
     */
    msiteur = "msiteur",
    /**
     * ### MORGAN STANLEY
     * *MS.US/USD*
     */
    msususd = "msususd",
    /**
     * ### MSCI INC
     * *MSCI.US/USD*
     */
    msciususd = "msciususd",
    /**
     * ### MICROSOFT CORP
     * *MSFT.US/USD*
     */
    msftususd = "msftususd",
    /**
     * ### ArcelorMittal
     * *MT.NL/EUR*
     */
    mtnleur = "mtnleur",
    /**
     * ### MATCH GROUP INC
     * *MTCH.US/USD*
     */
    mtchususd = "mtchususd",
    /**
     * ### METTLER-TOLEDO INTERNATIONAL
     * *MTD.US/USD*
     */
    mtdususd = "mtdususd",
    /**
     * ### VAIL RESORTS INC
     * *MTN.US/USD*
     */
    mtnususd = "mtnususd",
    /**
     * ### MTU AERO ENGINES AG
     * *MTX.DE/EUR*
     */
    mtxdeeur = "mtxdeeur",
    /**
     * ### MICRON TECHNOLOGY INC
     * *MU.US/USD*
     */
    muususd = "muususd",
    /**
     * ### Muenchener Rueckversicherungs AG
     * *MUV2.DE/EUR*
     */
    muv2deeur = "muv2deeur",
    /**
     * ### NEUROCRINE BIOSCIENCES INC
     * *NBIX.US/USD*
     */
    nbixususd = "nbixususd",
    /**
     * ### NORWEGIAN CRUISE LINE HOLDIN
     * *NCLH.US/USD*
     */
    nclhususd = "nclhususd",
    /**
     * ### Nordea Bank AB
     * *NDA.SE/SEK*
     */
    ndasesek = "ndasesek",
    /**
     * ### NORDSON CORP
     * *NDSN.US/USD*
     */
    ndsnususd = "ndsnususd",
    /**
     * ### NEXTERA ENERGY INC
     * *NEE.US/USD*
     */
    neeususd = "neeususd",
    /**
     * ### NEWMONT MINING CORP
     * *NEM.US/USD*
     */
    nemususd = "nemususd",
    /**
     * ### Neste OYJ
     * *NES1V.FI/EUR*
     */
    nes1vfieur = "nes1vfieur",
    /**
     * ### Nestle SA
     * *NESN.CH/CHF*
     */
    nesnchchf = "nesnchchf",
    /**
     * ### NETFLIX INC
     * *NFLX.US/USD*
     */
    nflxususd = "nflxususd",
    /**
     * ### National Grid PLC
     * *NG.GB/GBX*
     */
    nggbgbx = "nggbgbx",
    /**
     * ### Norsk Hydro ASA
     * *NHY.NO/NOK*
     */
    nhynonok = "nhynonok",
    /**
     * ### Nio INC
     * *NIO.US/USD*
     */
    nioususd = "nioususd",
    /**
     * ### NIKE INC
     * *NKE.US/USD*
     */
    nkeususd = "nkeususd",
    /**
     * ### Netherlands 25 Index
     * *NLD.IDX/EUR*
     */
    nldidxeur = "nldidxeur",
    /**
     * ### NORTONLIFELOCK INC
     * *NLOK.US/USD*
     */
    nlokususd = "nlokususd",
    /**
     * ### NIELSEN HOLDINGS PLC
     * *NLSN.US/USD*
     */
    nlsnususd = "nlsnususd",
    /**
     * ### ANNALY CAPITAL MANAGEMENT IN
     * *NLY.US/USD*
     */
    nlyususd = "nlyususd",
    /**
     * ### NORTHROP GRUMMAN CORP
     * *NOC.US/USD*
     */
    nocususd = "nocususd",
    /**
     * ### Novartis AG
     * *NOVN.CH/CHF*
     */
    novnchchf = "novnchchf",
    /**
     * ### Novo Nordisk A/S
     * *NOVOB.DK/DKK*
     */
    novobdkdkk = "novobdkdkk",
    /**
     * ### SERVICENOW INC
     * *NOW.US/USD*
     */
    nowususd = "nowususd",
    /**
     * ### Nokian Renkaat OYJ
     * *NRE1V.FI/EUR*
     */
    nre1vfieur = "nre1vfieur",
    /**
     * ### NRG ENERGY INC
     * *NRG.US/USD*
     */
    nrgususd = "nrgususd",
    /**
     * ### NORFOLK SOUTHERN CORP
     * *NSC.US/USD*
     */
    nscususd = "nscususd",
    /**
     * ### NUTANIX INC - A
     * *NTNX.US/USD*
     */
    ntnxususd = "ntnxususd",
    /**
     * ### NVIDIA CORP
     * *NVDA.US/USD*
     */
    nvdaususd = "nvdaususd",
    /**
     * ### NVR INC
     * *NVR.US/USD*
     */
    nvrususd = "nvrususd",
    /**
     * ### NEWELL BRANDS INC
     * *NWL.US/USD*
     */
    nwlususd = "nwlususd",
    /**
     * ### NEWS CORP - CLASS B
     * *NWS.US/USD*
     */
    nwsususd = "nwsususd",
    /**
     * ### NEXSTAR MEDIA GROUP INC-CL A
     * *NXST.US/USD*
     */
    nxstususd = "nxstususd",
    /**
     * ### Next PLC
     * *NXT.GB/GBX*
     */
    nxtgbgbx = "nxtgbgbx",
    /**
     * ### NEW YORK TIMES CO-A
     * *NYT.US/USD*
     */
    nytususd = "nytususd",
    /**
     * ### New Zealand Dollar vs Canadian Dollar
     * *NZD/CAD*
     */
    nzdcad = "nzdcad",
    /**
     * ### New Zealand Dollar vs Swiss Franc
     * *NZD/CHF*
     */
    nzdchf = "nzdchf",
    /**
     * ### New Zealand Dollar vs Japanese Yen
     * *NZD/JPY*
     */
    nzdjpy = "nzdjpy",
    /**
     * ### New Zealand Dollar vs US Dollar
     * *NZD/USD*
     */
    nzdusd = "nzdusd",
    /**
     * ### Novozymes A/S
     * *NZYMB.DK/DKK*
     */
    nzymbdkdkk = "nzymbdkdkk",
    /**
     * ### REALTY INCOME CORP
     * *O.US/USD*
     */
    oususd = "oususd",
    /**
     * ### OWENS CORNING
     * *OC.US/USD*
     */
    ocususd = "ocususd",
    /**
     * ### OCADO GROUP PLC
     * *OCDO.GB/GBX*
     */
    ocdogbgbx = "ocdogbgbx",
    /**
     * ### OLD DOMINION FREIGHT LINE
     * *ODFL.US/USD*
     */
    odflususd = "odflususd",
    /**
     * ### OC Oerlikon Corporation
     * *OERL.CH/CHF*
     */
    oerlchchf = "oerlchchf",
    /**
     * ### OGE ENERGY CORP
     * *OGE.US/USD*
     */
    ogeususd = "ogeususd",
    /**
     * ### ORGANON & CO
     * *OGN.US/USD*
     */
    ognususd = "ognususd",
    /**
     * ### OMEGA HEALTHCARE INVESTORS
     * *OHI.US/USD*
     */
    ohiususd = "ohiususd",
    /**
     * ### Orange Juice
     * *OJUICE.CMD/USX*
     */
    ojuicecmdusx = "ojuicecmdusx",
    /**
     * ### ONEOK INC
     * *OKE.US/USD*
     */
    okeususd = "okeususd",
    /**
     * ### OKTA INC
     * *OKTA.US/USD*
     */
    oktaususd = "oktaususd",
    /**
     * ### UNIVERSAL DISPLAY CORP
     * *OLED.US/USD*
     */
    oledususd = "oledususd",
    /**
     * ### OLLIE'S BARGAIN OUTLET HOLDI
     * *OLLI.US/USD*
     */
    olliususd = "olliususd",
    /**
     * ### OLIN CORP
     * *OLN.US/USD*
     */
    olnususd = "olnususd",
    /**
     * ### Grupo Aeroportuario del Centro Norte
     * *OMAB.MX/MXN*
     */
    omabmxmxn = "omabmxmxn",
    /**
     * ### OMNICOM GROUP
     * *OMC.US/USD*
     */
    omcususd = "omcususd",
    /**
     * ### ONEMAIN HOLDINGS INC
     * *OMF.US/USD*
     */
    omfususd = "omfususd",
    /**
     * ### ON SEMICONDUCTOR
     * *ON.US/USD*
     */
    onususd = "onususd",
    /**
     * ### L'Oreal SA
     * *OR.FR/EUR*
     */
    orfreur = "orfreur",
    /**
     * ### Orange SA
     * *ORA.FR/EUR*
     */
    orafreur = "orafreur",
    /**
     * ### Orbia Advance Corp SAB DE CV
     * *ORBIA.MX/MXN*
     */
    orbiamxmxn = "orbiamxmxn",
    /**
     * ### ORACLE CORP
     * *ORCL.US/USD*
     */
    orclususd = "orclususd",
    /**
     * ### Orkla ASA
     * *ORK.NO/NOK*
     */
    orknonok = "orknonok",
    /**
     * ### O'REILLY AUTOMOTIVE INC
     * *ORLY.US/USD*
     */
    orlyususd = "orlyususd",
    /**
     * ### Outotec OYJ
     * *OTE1V.FI/EUR*
     */
    ote1vfieur = "ote1vfieur",
    /**
     * ### OTIS WORLDWIDE CORP
     * *OTIS.US/USD*
     */
    otisususd = "otisususd",
    /**
     * ### Outokumpu OYJ
     * *OUT1V.FI/EUR*
     */
    out1vfieur = "out1vfieur",
    /**
     * ### OCCIDENTAL PETROLEUM CORP
     * *OXY.US/USD*
     */
    oxyususd = "oxyususd",
    /**
     * ### Porsche Automobil Holding SE
     * *PAH3.DE/EUR*
     */
    pah3deeur = "pah3deeur",
    /**
     * ### PALO ALTO NETWORKS INC
     * *PANW.US/USD*
     */
    panwususd = "panwususd",
    /**
     * ### PARAMOUNT GLOBAL-CLASS B
     * *PARA.US/USD*
     */
    paraususd = "paraususd",
    /**
     * ### PAYCOM SOFTWARE INC
     * *PAYC.US/USD*
     */
    paycususd = "paycususd",
    /**
     * ### PAYCHEX INC
     * *PAYX.US/USD*
     */
    payxususd = "payxususd",
    /**
     * ### PBF ENERGY INC-CLASS A
     * *PBF.US/USD*
     */
    pbfususd = "pbfususd",
    /**
     * ### Petroleo Brasileiro S.A.
     * *PBR.US/USD*
     */
    pbrususd = "pbrususd",
    /**
     * ### PACCAR INC
     * *PCAR.US/USD*
     */
    pcarususd = "pcarususd",
    /**
     * ### P G & E CORP
     * *PCG.US/USD*
     */
    pcgususd = "pcgususd",
    /**
     * ### PRICELINE GROUP INC/THE
     * *PCLN.US/USD*
     */
    pclnususd = "pclnususd",
    /**
     * ### PAYLOCITY HOLDING CORP
     * *PCTY.US/USD*
     */
    pctyususd = "pctyususd",
    /**
     * ### HEALTHPEAK PROPERTIES INC
     * *PEAK.US/USD*
     */
    peakususd = "peakususd",
    /**
     * ### PENN NATIONAL GAMING INC
     * *PENN.US/USD*
     */
    pennususd = "pennususd",
    /**
     * ### Industrias Penoles SAB DE CV
     * *PEOLES.MX/MXN*
     */
    peolesmxmxn = "peolesmxmxn",
    /**
     * ### PEPSICO INC
     * *PEP.US/USD*
     */
    pepususd = "pepususd",
    /**
     * ### Petrofac Ltd
     * *PFC.GB/GBX*
     */
    pfcgbgbx = "pfcgbgbx",
    /**
     * ### PFIZER INC
     * *PFE.US/USD*
     */
    pfeususd = "pfeususd",
    /**
     * ### PROCTER & GAMBLE CO/THE
     * *PG.US/USD*
     */
    pgususd = "pgususd",
    /**
     * ### PROGRESSIVE CORP
     * *PGR.US/USD*
     */
    pgrususd = "pgrususd",
    /**
     * ### PARKER HANNIFIN CORP
     * *PH.US/USD*
     */
    phususd = "phususd",
    /**
     * ### Koninklijke Philips NV
     * *PHIA.NL/EUR*
     */
    phianleur = "phianleur",
    /**
     * ### PHOENIX GROUP HOLDINGS PLC
     * *PHNX.GB/GBX*
     */
    phnxgbgbx = "phnxgbgbx",
    /**
     * ### Piaggio & C. Spa
     * *PIA.IT/EUR*
     */
    piaiteur = "piaiteur",
    /**
     * ### POLARIS INC
     * *PII.US/USD*
     */
    piiususd = "piiususd",
    /**
     * ### Promotora Y Operadora DE INF
     * *PINFRA.MX/MXN*
     */
    pinframxmxn = "pinframxmxn",
    /**
     * ### PACKAGING CORP OF AMERICA
     * *PKG.US/USD*
     */
    pkgususd = "pkgususd",
    /**
     * ### ANAPLAN INC
     * *PLAN.US/USD*
     */
    planususd = "planususd",
    /**
     * ### Poland 20 Index
     * *PLN.IDX/PLN*
     */
    plnidxpln = "plnidxpln",
    /**
     * ### PLANET FITNESS INC - CL A
     * *PLNT.US/USD*
     */
    plntususd = "plntususd",
    /**
     * ### Palantir Tech Inc A
     * *PLTR.US/USD*
     */
    pltrususd = "pltrususd",
    /**
     * ### Plug Power Inc
     * *PLUG.US/USD*
     */
    plugususd = "plugususd",
    /**
     * ### PHILIP MORRIS INTERNATIONAL
     * *PM.US/USD*
     */
    pmususd = "pmususd",
    /**
     * ### PNC FINANCIAL SERVICES GROUP
     * *PNC.US/USD*
     */
    pncususd = "pncususd",
    /**
     * ### Pandora A/S
     * *PNDORA.DK/DKK*
     */
    pndoradkdkk = "pndoradkdkk",
    /**
     * ### PENTAIR PLC
     * *PNR.US/USD*
     */
    pnrususd = "pnrususd",
    /**
     * ### INSULET CORP
     * *PODD.US/USD*
     */
    poddususd = "poddususd",
    /**
     * ### POOL CORP
     * *POOL.US/USD*
     */
    poolususd = "poolususd",
    /**
     * ### PPG INDUSTRIES INC
     * *PPG.US/USD*
     */
    ppgususd = "ppgususd",
    /**
     * ### PERRIGO CO PLC
     * *PRGO.US/USD*
     */
    prgoususd = "prgoususd",
    /**
     * ### Prudential PLC
     * *PRU.GB/GBX*
     */
    prugbgbx = "prugbgbx",
    /**
     * ### PRUDENTIAL FINANCIAL INC
     * *PRU.US/USD*
     */
    pruususd = "pruususd",
    /**
     * ### Prysmian Spa
     * *PRY.IT/EUR*
     */
    pryiteur = "pryiteur",
    /**
     * ### PUBLIC STORAGE
     * *PSA.US/USD*
     */
    psaususd = "psaususd",
    /**
     * ### ProSiebenSat.1 Media AG
     * *PSM.DE/EUR*
     */
    psmdeeur = "psmdeeur",
    /**
     * ### Persimmon PLC
     * *PSN.GB/GBX*
     */
    psngbgbx = "psngbgbx",
    /**
     * ### Pearson PLC
     * *PSON.GB/GBX*
     */
    psongbgbx = "psongbgbx",
    /**
     * ### PURE STORAGE INC - CLASS A
     * *PSTG.US/USD*
     */
    pstgususd = "pstgususd",
    /**
     * ### PHILLIPS 66
     * *PSX.US/USD*
     */
    psxususd = "psxususd",
    /**
     * ### PTC INC
     * *PTC.US/USD*
     */
    ptcususd = "ptcususd",
    /**
     * ### PATTERSON-UTI ENERGY INC
     * *PTEN.US/USD*
     */
    ptenususd = "ptenususd",
    /**
     * ### Peloton Interactive Inc
     * *PTON.US/USD*
     */
    ptonususd = "ptonususd",
    /**
     * ### Publicis Groupe SA
     * *PUB.FR/EUR*
     */
    pubfreur = "pubfreur",
    /**
     * ### PUMA SE
     * *PUM.DE/EUR*
     */
    pumdeeur = "pumdeeur",
    /**
     * ### PVH CORP
     * *PVH.US/USD*
     */
    pvhususd = "pvhususd",
    /**
     * ### PIONEER NATURAL RESOURCES CO
     * *PXD.US/USD*
     */
    pxdususd = "pxdususd",
    /**
     * ### PAYPAL HOLDINGS INC
     * *PYPL.US/USD*
     */
    pyplususd = "pyplususd",
    /**
     * ### Qualitas Controladora SAB
     * *Q.MX/MXN*
     */
    qmxmxn = "qmxmxn",
    /**
     * ### QUALCOMM INC
     * *QCOM.US/USD*
     */
    qcomususd = "qcomususd",
    /**
     * ### QIAGEN N.V.
     * *QIA.NL/EUR*
     */
    qianleur = "qianleur",
    /**
     * ### PowerShares QQQ ETF
     * *QQQ.US/USD*
     */
    qqqususd = "qqqususd",
    /**
     * ### QORVO INC
     * *QRVO.US/USD*
     */
    qrvoususd = "qrvoususd",
    /**
     * ### Regional Sab De CV
     * *RA.MX/MXN*
     */
    ramxmxn = "ramxmxn",
    /**
     * ### Ferrari NV
     * *RACE.IT/EUR*
     */
    raceiteur = "raceiteur",
    /**
     * ### Randstad Holding NV
     * *RAND.NL/EUR*
     */
    randnleur = "randnleur",
    /**
     * ### Reckitt Benckiser Group PLC
     * *RB.GB/GBX*
     */
    rbgbgbx = "rbgbgbx",
    /**
     * ### Raiffeisen Bank International AG
     * *RBI.AT/EUR*
     */
    rbiateur = "rbiateur",
    /**
     * ### Royal Bank of Scotland Group PLC
     * *RBS.GB/GBX*
     */
    rbsgbgbx = "rbsgbgbx",
    /**
     * ### ROYAL CARIBBEAN CRUISES LTD
     * *RCL.US/USD*
     */
    rclususd = "rclususd",
    /**
     * ### Royal Dutch Shell PLC
     * *RDSA.NL/EUR*
     */
    rdsanleur = "rdsanleur",
    /**
     * ### Royal Dutch Shell PLC
     * *RDSB.GB/GBX*
     */
    rdsbgbgbx = "rdsbgbgbx",
    /**
     * ### EVEREST RE GROUP LTD
     * *RE.US/USD*
     */
    reususd = "reususd",
    /**
     * ### Recordati Industria Chimica e Farma Spa
     * *REC.IT/EUR*
     */
    reciteur = "reciteur",
    /**
     * ### Red Electrica Corp SA
     * *REE.ES/EUR*
     */
    reeeseur = "reeeseur",
    /**
     * ### REGENCY CENTERS CORP
     * *REG.US/USD*
     */
    regususd = "regususd",
    /**
     * ### REGENERON PHARMACEUTICALS
     * *REGN.US/USD*
     */
    regnususd = "regnususd",
    /**
     * ### Reed Elsevier PLC
     * *REL.GB/GBX*
     */
    relgbgbx = "relgbgbx",
    /**
     * ### RELX NV
     * *REN.NL/EUR*
     */
    rennleur = "rennleur",
    /**
     * ### Repsol SA
     * *REP.ES/EUR*
     */
    repeseur = "repeseur",
    /**
     * ### REGIONS FINANCIAL CORP
     * *RF.US/USD*
     */
    rfususd = "rfususd",
    /**
     * ### REINSURANCE GROUP OF AMERICA
     * *RGA.US/USD*
     */
    rgaususd = "rgaususd",
    /**
     * ### ROYAL GOLD INC
     * *RGLD.US/USD*
     */
    rgldususd = "rgldususd",
    /**
     * ### Pernod-Ricard SA
     * *RI.FR/EUR*
     */
    rifreur = "rifreur",
    /**
     * ### Rio Tinto PLC
     * *RIO.GB/GBX*
     */
    riogbgbx = "riogbgbx",
    /**
     * ### RAYMOND JAMES FINANCIAL INC
     * *RJF.US/USD*
     */
    rjfususd = "rjfususd",
    /**
     * ### RESMED INC
     * *RMD.US/USD*
     */
    rmdususd = "rmdususd",
    /**
     * ### Royal Mail PLC
     * *RMG.GB/GBX*
     */
    rmggbgbx = "rmggbgbx",
    /**
     * ### HERMES INTERNATIONAL
     * *RMS.FR/EUR*
     */
    rmsfreur = "rmsfreur",
    /**
     * ### RIGHTMOVE PLC
     * *RMV.GB/GBX*
     */
    rmvgbgbx = "rmvgbgbx",
    /**
     * ### RINGCENTRAL INC-CLASS A
     * *RNG.US/USD*
     */
    rngususd = "rngususd",
    /**
     * ### Renault SA
     * *RNO.FR/EUR*
     */
    rnofreur = "rnofreur",
    /**
     * ### RENAISSANCERE HOLDINGS LTD
     * *RNR.US/USD*
     */
    rnrususd = "rnrususd",
    /**
     * ### Roche Holding AG
     * *ROG.CH/CHF*
     */
    rogchchf = "rogchchf",
    /**
     * ### Roku Inc
     * *ROKU.US/USD*
     */
    rokuususd = "rokuususd",
    /**
     * ### ROLLINS INC
     * *ROL.US/USD*
     */
    rolususd = "rolususd",
    /**
     * ### ROSS STORES INC
     * *ROST.US/USD*
     */
    rostususd = "rostususd",
    /**
     * ### RPM INTERNATIONAL INC
     * *RPM.US/USD*
     */
    rpmususd = "rpmususd",
    /**
     * ### Rolls-Royce Holdings PLC
     * *RR.GB/GBX*
     */
    rrgbgbx = "rrgbgbx",
    /**
     * ### RANGE RESOURCES CORP
     * *RRC.US/USD*
     */
    rrcususd = "rrcususd",
    /**
     * ### REGAL REXNORD CORP
     * *RRX.US/USD*
     */
    rrxususd = "rrxususd",
    /**
     * ### RELIANCE STEEL & ALUMINUM
     * *RS.US/USD*
     */
    rsususd = "rsususd",
    /**
     * ### RSA Insurance Group PLC
     * *RSA.GB/GBX*
     */
    rsagbgbx = "rsagbgbx",
    /**
     * ### RENTOKIL INITIAL PLC
     * *RTO.GB/GBX*
     */
    rtogbgbx = "rtogbgbx",
    /**
     * ### RAYTHEON TECHNOLOGIES CORP
     * *RTX.US/USD*
     */
    rtxususd = "rtxususd",
    /**
     * ### RWE AG
     * *RWE.DE/EUR*
     */
    rwedeeur = "rwedeeur",
    /**
     * ### Ryanair Holdings Plc
     * *RY4C.IE/EUR*
     */
    ry4cieeur = "ry4cieeur",
    /**
     * ### SABRE CORP
     * *SABR.US/USD*
     */
    sabrususd = "sabrususd",
    /**
     * ### Safran SA
     * *SAF.FR/EUR*
     */
    saffreur = "saffreur",
    /**
     * ### Banco Santander SA
     * *SAN.ES/EUR*
     */
    saneseur = "saneseur",
    /**
     * ### Sanofi
     * *SAN.FR/EUR*
     */
    sanfreur = "sanfreur",
    /**
     * ### Sandvik AB
     * *SAND.SE/SEK*
     */
    sandsesek = "sandsesek",
    /**
     * ### SAP AG
     * *SAP.DE/EUR*
     */
    sapdeeur = "sapdeeur",
    /**
     * ### SIGNATURE BANK
     * *SBNY.US/USD*
     */
    sbnyususd = "sbnyususd",
    /**
     * ### J Sainsbury PLC
     * *SBRY.GB/GBX*
     */
    sbrygbgbx = "sbrygbgbx",
    /**
     * ### STARBUCKS CORP
     * *SBUX.US/USD*
     */
    sbuxususd = "sbuxususd",
    /**
     * ### Svenska Cellulosa AB
     * *SCAB.SE/SEK*
     */
    scabsesek = "scabsesek",
    /**
     * ### SOUTHERN COPPER CORP
     * *SCCO.US/USD*
     */
    sccoususd = "sccoususd",
    /**
     * ### SCHWAB (CHARLES) CORP
     * *SCHW.US/USD*
     */
    schwususd = "schwususd",
    /**
     * ### SERVICE CORP INTERNATIONAL
     * *SCI.US/USD*
     */
    sciususd = "sciususd",
    /**
     * ### Swisscom AG
     * *SCMN.CH/CHF*
     */
    scmnchchf = "scmnchchf",
    /**
     * ### K+S AG
     * *SDF.DE/EUR*
     */
    sdfdeeur = "sdfdeeur",
    /**
     * ### Schroders PLC
     * *SDR.GB/GBX*
     */
    sdrgbgbx = "sdrgbgbx",
    /**
     * ### Skandinaviska Enskilda Banken AB
     * *SEBA.SE/SEK*
     */
    sebasesek = "sebasesek",
    /**
     * ### Securitas AB
     * *SECUB.SE/SEK*
     */
    secubsesek = "secubsesek",
    /**
     * ### SOLAREDGE TECHNOLOGIES INC
     * *SEDG.US/USD*
     */
    sedgususd = "sedgususd",
    /**
     * ### SEI INVESTMENTS COMPANY
     * *SEIC.US/USD*
     */
    seicususd = "seicususd",
    /**
     * ### Salvat Ferragamo Spa
     * *SFER.IT/EUR*
     */
    sferiteur = "sferiteur",
    /**
     * ### SPROUTS FARMERS MARKET INC
     * *SFM.US/USD*
     */
    sfmususd = "sfmususd",
    /**
     * ### Singapore Blue Chip Cash Index
     * *SGD.IDX/SGD*
     */
    sgdidxsgd = "sgdidxsgd",
    /**
     * ### Singapore Dollar vs Japanese Yen
     * *SGD/JPY*
     */
    sgdjpy = "sgdjpy",
    /**
     * ### Sage Group PLC/The
     * *SGE.GB/GBX*
     */
    sgegbgbx = "sgegbgbx",
    /**
     * ### SEAGEN INC
     * *SGEN.US/USD*
     */
    sgenususd = "sgenususd",
    /**
     * ### Cie de St-Gobain
     * *SGO.FR/EUR*
     */
    sgofreur = "sgofreur",
    /**
     * ### SEGRO PLC
     * *SGRO.GB/GBX*
     */
    sgrogbgbx = "sgrogbgbx",
    /**
     * ### SGS SA
     * *SGSN.CH/CHF*
     */
    sgsnchchf = "sgsnchchf",
    /**
     * ### SHERWIN-WILLIAMS COMPANY
     * *SHW.US/USD*
     */
    shwususd = "shwususd",
    /**
     * ### Siemens AG
     * *SIE.DE/EUR*
     */
    siedeeur = "siedeeur",
    /**
     * ### SVB FINANCIAL GROUP
     * *SIVB.US/USD*
     */
    sivbususd = "sivbususd",
    /**
     * ### JM SMUCKER COMPANY
     * *SJM.US/USD*
     */
    sjmususd = "sjmususd",
    /**
     * ### Skanska AB
     * *SKAB.SE/SEK*
     */
    skabsesek = "skabsesek",
    /**
     * ### SKF AB
     * *SKFB.SE/SEK*
     */
    skfbsesek = "skfbsesek",
    /**
     * ### Sky Plc
     * *SKY.GB/GBP*
     */
    skygbgbx = "skygbgbx",
    /**
     * ### Standard Life Aberdeen Plc
     * *SLA.GB/GBX*
     */
    slagbgbx = "slagbgbx",
    /**
     * ### SCHLUMBERGER LTD
     * *SLB.US/USD*
     */
    slbususd = "slbususd",
    /**
     * ### Swiss Life Holding AG
     * *SLHN.CH/CHF*
     */
    slhnchchf = "slhnchchf",
    /**
     * ### SLM CORP
     * *SLM.US/USD*
     */
    slmususd = "slmususd",
    /**
     * ### iShares Silver Trust ETF
     * *SLV.US/USD*
     */
    slvususd = "slvususd",
    /**
     * ### DS SMITH PLC
     * *SMDS.GB/GBX*
     */
    smdsgbgbx = "smdsgbgbx",
    /**
     * ### Smiths Group PLC
     * *SMIN.GB/GBX*
     */
    smingbgbx = "smingbgbx",
    /**
     * ### SCOTTISH MORTGAGE INV TR PLC
     * *SMT.GB/GBX*
     */
    smtgbgbx = "smtgbgbx",
    /**
     * ### Smith & Nephew PLC
     * *SN.GB/GBX*
     */
    sngbgbx = "sngbgbx",
    /**
     * ### SNAP Inc
     * *SNAP.US/USD*
     */
    snapususd = "snapususd",
    /**
     * ### Snowflake Inc Class A
     * *SNOW.US/USD*
     */
    snowususd = "snowususd",
    /**
     * ### SYNOPSYS INC
     * *SNPS.US/USD*
     */
    snpsususd = "snpsususd",
    /**
     * ### SOUTHERN CO/THE
     * *SO.US/USD*
     */
    soususd = "soususd",
    /**
     * ### South Africa 40 Index
     * *SOA.IDX/ZAR*
     */
    soaidxzar = "soaidxzar",
    /**
     * ### Solvay SA
     * *SOLB.BE/EUR*
     */
    solbbeeur = "solbbeeur",
    /**
     * ### Sonova Holding AG
     * *SOON.CH/CHF*
     */
    soonchchf = "soonchchf",
    /**
     * ### Soybean
     * *SOYBEAN.CMD/USX*
     */
    soybeancmdusx = "soybeancmdusx",
    /**
     * ### SIMON PROPERTY GROUP INC
     * *SPG.US/USD*
     */
    spgususd = "spgususd",
    /**
     * ### S&P GLOBAL INC
     * *SPGI.US/USD*
     */
    spgiususd = "spgiususd",
    /**
     * ### SPLUNK INC
     * *SPLK.US/USD*
     */
    splkususd = "splkususd",
    /**
     * ### SAIPEM Spa
     * *SPM.IT/EUR*
     */
    spmiteur = "spmiteur",
    /**
     * ### SPOTIFY TECHNOLOGY SA
     * *SPOT.US/USD*
     */
    spotususd = "spotususd",
    /**
     * ### SPIRIT AEROSYSTEMS HOLD-CL A
     * *SPR.US/USD*
     */
    sprususd = "sprususd",
    /**
     * ### SPIRAX-SARCO ENGINEERING PLC
     * *SPX.GB/GBX*
     */
    spxgbgbx = "spxgbgbx",
    /**
     * ### SPDR S&P 500 ETF
     * *SPY.US/USD*
     */
    spyususd = "spyususd",
    /**
     * ### Square Inc
     * *SQ.US/USD*
     */
    sqususd = "sqususd",
    /**
     * ### Swiss Re AG
     * *SREN.CH/CHF*
     */
    srenchchf = "srenchchf",
    /**
     * ### Snam Spa
     * *SRG.IT/EUR*
     */
    srgiteur = "srgiteur",
    /**
     * ### SAREPTA THERAPEUTICS INC
     * *SRPT.US/USD*
     */
    srptususd = "srptususd",
    /**
     * ### Saras Spa
     * *SRS.IT/EUR*
     */
    srsiteur = "srsiteur",
    /**
     * ### Sartorius AG - Vorzug
     * *SRT.DE/EUR*
     */
    srtdeeur = "srtdeeur",
    /**
     * ### SSE PLC
     * *SSE.GB/GBX*
     */
    ssegbgbx = "ssegbgbx",
    /**
     * ### SS&C TECHNOLOGIES HOLDINGS
     * *SSNC.US/USD*
     */
    ssncususd = "ssncususd",
    /**
     * ### Standard Chartered PLC
     * *STAN.GB/GBX*
     */
    stangbgbx = "stangbgbx",
    /**
     * ### STERIS PLC
     * *STE.US/USD*
     */
    steususd = "steususd",
    /**
     * ### Stora Enso OYJ
     * *STERV.FI/EUR*
     */
    stervfieur = "stervfieur",
    /**
     * ### ST JAMES'S PLACE PLC
     * *STJ.GB/GBX*
     */
    stjgbgbx = "stjgbgbx",
    /**
     * ### Statoil ASA
     * *STL.NO/NOK*
     */
    stlnonok = "stlnonok",
    /**
     * ### STMicroelectronics NV
     * *STM.IT/EUR*
     */
    stmiteur = "stmiteur",
    /**
     * ### STATE STREET CORP
     * *STT.US/USD*
     */
    sttususd = "sttususd",
    /**
     * ### CONSTELLATION BRANDS INC-A
     * *STZ.US/USD*
     */
    stzususd = "stzususd",
    /**
     * ### Schneider Electric SA
     * *SU.FR/EUR*
     */
    sufreur = "sufreur",
    /**
     * ### London Sugar No.5
     * *SUGAR.CMD/USD*
     */
    sugarcmdusd = "sugarcmdusd",
    /**
     * ### SUN COMMUNITIES INC
     * *SUI.US/USD*
     */
    suiususd = "suiususd",
    /**
     * ### Severn Trent PLC
     * *SVT.GB/GBX*
     */
    svtgbgbx = "svtgbgbx",
    /**
     * ### SODEXO SA
     * *SW.FR/EUR*
     */
    swfreur = "swfreur",
    /**
     * ### Swedbank AB
     * *SWEDA.SE/SEK*
     */
    swedasesek = "swedasesek",
    /**
     * ### STANLEY BLACK & DECKER INC
     * *SWK.US/USD*
     */
    swkususd = "swkususd",
    /**
     * ### SKYWORKS SOLUTIONS INC
     * *SWKS.US/USD*
     */
    swksususd = "swksususd",
    /**
     * ### Swedish Match AB
     * *SWMA.SE/SEK*
     */
    swmasesek = "swmasesek",
    /**
     * ### SYMRISE AG
     * *SY1.DE/EUR*
     */
    sy1deeur = "sy1deeur",
    /**
     * ### SYNCHRONY FINANCIAL
     * *SYF.US/USD*
     */
    syfususd = "syfususd",
    /**
     * ### STRYKER CORP
     * *SYK.US/USD*
     */
    sykususd = "sykususd",
    /**
     * ### SYSCO CORP
     * *SYY.US/USD*
     */
    syyususd = "syyususd",
    /**
     * ### AT&T INC
     * *T.US/USD*
     */
    tususd = "tususd",
    /**
     * ### MOLSON COORS BREWING CO -B
     * *TAP.US/USD*
     */
    tapususd = "tapususd",
    /**
     * ### Tate & Lyle PLC
     * *TATE.GB/GBX*
     */
    tategbgbx = "tategbgbx",
    /**
     * ### TRANSDIGM GROUP INC
     * *TDG.US/USD*
     */
    tdgususd = "tdgususd",
    /**
     * ### TELEDYNE TECHNOLOGIES INC
     * *TDY.US/USD*
     */
    tdyususd = "tdyususd",
    /**
     * ### ATLASSIAN CORP PLC-CLASS A
     * *TEAM.US/USD*
     */
    teamususd = "teamususd",
    /**
     * ### iShares TecDAX UCITS DE ETF
     * *TECDAXE.DE/EUR*
     */
    tecdaxedeeur = "tecdaxedeeur",
    /**
     * ### BIO-TECHNE CORP
     * *TECH.US/USD*
     */
    techususd = "techususd",
    /**
     * ### Telefonica SA
     * *TEF.ES/EUR*
     */
    tefeseur = "tefeseur",
    /**
     * ### Telenor ASA
     * *TEL.NO/NOK*
     */
    telnonok = "telnonok",
    /**
     * ### TE CONNECTIVITY LTD
     * *TEL.US/USD*
     */
    telususd = "telususd",
    /**
     * ### Tele2 AB
     * *TEL2B.SE/SEK*
     */
    tel2bsesek = "tel2bsesek",
    /**
     * ### Temenos Group AG
     * *TEMN.CH/CHF*
     */
    temnchchf = "temnchchf",
    /**
     * ### Tenaris SA
     * *TEN.IT/EUR*
     */
    teniteur = "teniteur",
    /**
     * ### TEVA PHARMACEUTICAL-SP ADR
     * *TEVA.US/USD*
     */
    tevaususd = "tevaususd",
    /**
     * ### TRUIST FINANCIAL CORP
     * *TFC.US/USD*
     */
    tfcususd = "tfcususd",
    /**
     * ### TELEFLEX INC
     * *TFX.US/USD*
     */
    tfxususd = "tfxususd",
    /**
     * ### TARGET CORP
     * *TGT.US/USD*
     */
    tgtususd = "tgtususd",
    /**
     * ### Tiscali Spa
     * *TIS.IT/EUR*
     */
    tisiteur = "tisiteur",
    /**
     * ### Telecom Italia Spa
     * *TIT.IT/EUR*
     */
    tititeur = "tititeur",
    /**
     * ### TJX COMPANIES INC
     * *TJX.US/USD*
     */
    tjxususd = "tjxususd",
    /**
     * ### ThyssenKrupp AG
     * *TKA.DE/EUR*
     */
    tkadeeur = "tkadeeur",
    /**
     * ### Telia Company AB
     * *TLS1V.FI/EUR*
     */
    tls1vfieur = "tls1vfieur",
    /**
     * ### Telia Company AB
     * *TLSN.SE/SEK*
     */
    tlsnsesek = "tlsnsesek",
    /**
     * ### iShares 20+ Year Treasury Bond ETF
     * *TLT.US/USD*
     */
    tltususd = "tltususd",
    /**
     * ### Tullow Oil PLC
     * *TLW.GB/GBX*
     */
    tlwgbgbx = "tlwgbgbx",
    /**
     * ### THERMO FISHER SCIENTIFIC INC
     * *TMO.US/USD*
     */
    tmoususd = "tmoususd",
    /**
     * ### T-MOBILE US INC
     * *TMUS.US/USD*
     */
    tmusususd = "tmusususd",
    /**
     * ### Tods Spa
     * *TOD.IT/EUR*
     */
    toditeur = "toditeur",
    /**
     * ### TOLL BROTHERS INC
     * *TOL.US/USD*
     */
    tolususd = "tolususd",
    /**
     * ### Travis Perkins PLC
     * *TPK.GB/GBX*
     */
    tpkgbgbx = "tpkgbgbx",
    /**
     * ### TAPESTRY INC
     * *TPR.US/USD*
     */
    tprususd = "tprususd",
    /**
     * ### TEMPUR SEALY INTERNATIONAL I
     * *TPX.US/USD*
     */
    tpxususd = "tpxususd",
    /**
     * ### TARGA RESOURCES CORP
     * *TRGP.US/USD*
     */
    trgpususd = "trgpususd",
    /**
     * ### TRIMBLE INC
     * *TRMB.US/USD*
     */
    trmbususd = "trmbususd",
    /**
     * ### Terna Spa
     * *TRN.IT/EUR*
     */
    trniteur = "trniteur",
    /**
     * ### T ROWE PRICE GROUP INC
     * *TROW.US/USD*
     */
    trowususd = "trowususd",
    /**
     * ### TRANSUNION
     * *TRU.US/USD*
     */
    truususd = "truususd",
    /**
     * ### TRAVELERS COS INC/THE
     * *TRV.US/USD*
     */
    trvususd = "trvususd",
    /**
     * ### TRON vs US Dollar
     * *TRX/USD*
     */
    trxusd = "trxusd",
    /**
     * ### Turkish Lira vs Japanese Yen
     * *TRY/JPY*
     */
    tryjpy = "tryjpy",
    /**
     * ### Tesco PLC
     * *TSCO.GB/GBX*
     */
    tscogbgbx = "tscogbgbx",
    /**
     * ### TESLA MOTORS INC
     * *TSLA.US/USD*
     */
    tslaususd = "tslaususd",
    /**
     * ### Taiwan Semiconductor Manufacturing Company Limited
     * *TSM.US/USD*
     */
    tsmususd = "tsmususd",
    /**
     * ### TYSON FOODS INC-CL A
     * *TSN.US/USD*
     */
    tsnususd = "tsnususd",
    /**
     * ### TRANE TECHNOLOGIES PLC
     * *TT.US/USD*
     */
    ttususd = "ttususd",
    /**
     * ### TRADE DESK INC/THE -CLASS A
     * *TTD.US/USD*
     */
    ttdususd = "ttdususd",
    /**
     * ### TAKE-TWO INTERACTIVE SOFTWRE
     * *TTWO.US/USD*
     */
    ttwoususd = "ttwoususd",
    /**
     * ### TUI AG
     * *TUI1.DE/EUR*
     */
    tui1deeur = "tui1deeur",
    /**
     * ### TAYLOR WIMPEY PLC
     * *TW.GB/GBX*
     */
    twgbgbx = "twgbgbx",
    /**
     * ### TWILIO INC - A
     * *TWLO.US/USD*
     */
    twloususd = "twloususd",
    /**
     * ### TWITTER INC
     * *TWTR.US/USD*
     */
    twtrususd = "twtrususd",
    /**
     * ### TEXAS INSTRUMENTS INC
     * *TXN.US/USD*
     */
    txnususd = "txnususd",
    /**
     * ### TYLER TECHNOLOGIES INC
     * *TYL.US/USD*
     */
    tylususd = "tylususd",
    /**
     * ### UNDER ARMOUR INC-CLASS C
     * *UA.US/USD*
     */
    uaususd = "uaususd",
    /**
     * ### UNDER ARMOUR INC-CLASS A
     * *UAA.US/USD*
     */
    uaaususd = "uaaususd",
    /**
     * ### Uber Inc
     * *UBER.US/USD*
     */
    uberususd = "uberususd",
    /**
     * ### UBS Group AG
     * *UBSG.CH/CHF*
     */
    ubsgchchf = "ubsgchchf",
    /**
     * ### UCB SA
     * *UCB.BE/EUR*
     */
    ucbbeeur = "ucbbeeur",
    /**
     * ### UniCredit Spa
     * *UCG.IT/EUR*
     */
    ucgiteur = "ucgiteur",
    /**
     * ### UDR INC
     * *UDR.US/USD*
     */
    udrususd = "udrususd",
    /**
     * ### Peugeot SA
     * *UG.FR/EUR*
     */
    ugfreur = "ugfreur",
    /**
     * ### UGI CORP
     * *UGI.US/USD*
     */
    ugiususd = "ugiususd",
    /**
     * ### Swatch Group AG/The
     * *UHR.CH/CHF*
     */
    uhrchchf = "uhrchchf",
    /**
     * ### Swatch Group AG (The Registered)
     * *UHRN.CH/CHF*
     */
    uhrnchchf = "uhrnchchf",
    /**
     * ### UNIVERSAL HEALTH SERVICES-B
     * *UHS.US/USD*
     */
    uhsususd = "uhsususd",
    /**
     * ### UK Long Gilt
     * *UKGILT.TR/GBP*
     */
    ukgilttrgbp = "ukgilttrgbp",
    /**
     * ### ULTA SALON COSMETICS & FRAGR
     * *ULTA.US/USD*
     */
    ultaususd = "ultaususd",
    /**
     * ### Unilever PLC
     * *ULVR.GB/GBX*
     */
    ulvrgbgbx = "ulvrgbgbx",
    /**
     * ### Umicore SA
     * *UMI.BE/EUR*
     */
    umibeeur = "umibeeur",
    /**
     * ### Unilever NV
     * *UNA.NL/EUR*
     */
    unanleur = "unanleur",
    /**
     * ### UNITEDHEALTH GROUP INC
     * *UNH.US/USD*
     */
    unhususd = "unhususd",
    /**
     * ### Uniswap vs US Dollar
     * *UNI/USD*
     */
    uniusd = "uniusd",
    /**
     * ### UNION PACIFIC CORP
     * *UNP.US/USD*
     */
    unpususd = "unpususd",
    /**
     * ### UNITED PARCEL SERVICE-CL B
     * *UPS.US/USD*
     */
    upsususd = "upsususd",
    /**
     * ### UNITED RENTALS INC
     * *URI.US/USD*
     */
    uriususd = "uriususd",
    /**
     * ### Unipolsai Assicurazioni Spa
     * *US.IT/EUR*
     */
    usiteur = "usiteur",
    /**
     * ### US BANCORP
     * *USB.US/USD*
     */
    usbususd = "usbususd",
    /**
     * ### US Dollar vs Canadian Dollar
     * *USD/CAD*
     */
    usdcad = "usdcad",
    /**
     * ### US Dollar vs Swiss Franc
     * *USD/CHF*
     */
    usdchf = "usdchf",
    /**
     * ### US Dollar vs Offshore Chinese Renminbi
     * *USD/CNH*
     */
    usdcnh = "usdcnh",
    /**
     * ### US Dollar vs Czech Koruna
     * *USD/CZK*
     */
    usdczk = "usdczk",
    /**
     * ### US Dollar vs Danish Krone
     * *USD/DKK*
     */
    usddkk = "usddkk",
    /**
     * ### US Dollar vs Hong Kong Dollar
     * *USD/HKD*
     */
    usdhkd = "usdhkd",
    /**
     * ### US Dollar vs Hungarian Forint
     * *USD/HUF*
     */
    usdhuf = "usdhuf",
    /**
     * ### US Dollar vs Israeli Shekel
     * *USD/ILS*
     */
    usdils = "usdils",
    /**
     * ### US Dollar vs Japanese Yen
     * *USD/JPY*
     */
    usdjpy = "usdjpy",
    /**
     * ### US Dollar vs Mexican Peso
     * *USD/MXN*
     */
    usdmxn = "usdmxn",
    /**
     * ### US Dollar vs Norwegian Krone
     * *USD/NOK*
     */
    usdnok = "usdnok",
    /**
     * ### US Dollar vs Polish Zloty
     * *USD/PLN*
     */
    usdpln = "usdpln",
    /**
     * ### US Dollar vs Romanian Leu
     * *USD/RON*
     */
    usdron = "usdron",
    /**
     * ### US Dollar vs Swedish Krona
     * *USD/SEK*
     */
    usdsek = "usdsek",
    /**
     * ### US Dollar vs Singapore Dollar
     * *USD/SGD*
     */
    usdsgd = "usdsgd",
    /**
     * ### US Dollar vs Thai Baht
     * *USD/THB*
     */
    usdthb = "usdthb",
    /**
     * ### US Dollar vs Turkish Lira
     * *USD/TRY*
     */
    usdtry = "usdtry",
    /**
     * ### US Dollar vs South African Rand
     * *USD/ZAR*
     */
    usdzar = "usdzar",
    /**
     * ### US FOODS HOLDING CORP
     * *USFD.US/USD*
     */
    usfdususd = "usfdususd",
    /**
     * ### United States Oil
     * *USO.US/USD*
     */
    usoususd = "usoususd",
    /**
     * ### US Small Cap 2000
     * *USSC2000.IDX/USD*
     */
    ussc2000idxusd = "ussc2000idxusd",
    /**
     * ### US T-Bond
     * *USTBOND.TR/USD*
     */
    ustbondtrusd = "ustbondtrusd",
    /**
     * ### UNITED THERAPEUTICS CORP
     * *UTHR.US/USD*
     */
    uthrususd = "uthrususd",
    /**
     * ### United Utilities Group PLC
     * *UU.GB/GBX*
     */
    uugbgbx = "uugbgbx",
    /**
     * ### VISA INC-CLASS A SHARES
     * *V.US/USD*
     */
    vususd = "vususd",
    /**
     * ### Vale S.A.
     * *VALE.US/USD*
     */
    valeususd = "valeususd",
    /**
     * ### Vanguard FTSE Developed Markets ETF
     * *VEA.US/USD*
     */
    veaususd = "veaususd",
    /**
     * ### VEEVA SYSTEMS INC-CLASS A
     * *VEEV.US/USD*
     */
    veevususd = "veevususd",
    /**
     * ### VF CORP
     * *VFC.US/USD*
     */
    vfcususd = "vfcususd",
    /**
     * ### Vanguard FTSE Europe ETF
     * *VGK.US/USD*
     */
    vgkususd = "vgkususd",
    /**
     * ### VICI PROPERTIES INC
     * *VICI.US/USD*
     */
    viciususd = "viciususd",
    /**
     * ### Veolia Environnement SA
     * *VIE.FR/EUR*
     */
    viefreur = "viefreur",
    /**
     * ### Vifor Pharma
     * *VIFN.CH/CHF*
     */
    vifnchchf = "vifnchchf",
    /**
     * ### VIRTU FINANCIAL INC-CLASS A
     * *VIRT.US/USD*
     */
    virtususd = "virtususd",
    /**
     * ### Viscofan SA
     * *VIS.ES/EUR*
     */
    viseseur = "viseseur",
    /**
     * ### Vivendi SA
     * *VIV.FR/EUR*
     */
    vivfreur = "vivfreur",
    /**
     * ### Vallourec SA
     * *VK.FR/EUR*
     */
    vkfreur = "vkfreur",
    /**
     * ### VALERO ENERGY CORP
     * *VLO.US/USD*
     */
    vloususd = "vloususd",
    /**
     * ### VULCAN MATERIALS CO
     * *VMC.US/USD*
     */
    vmcususd = "vmcususd",
    /**
     * ### VMWARE INC-CLASS A
     * *VMW.US/USD*
     */
    vmwususd = "vmwususd",
    /**
     * ### Vonovia SE
     * *VNA.DE/EUR*
     */
    vnadeeur = "vnadeeur",
    /**
     * ### Vanguard REIT ETF
     * *VNQ.US/USD*
     */
    vnqususd = "vnqususd",
    /**
     * ### Vodafone Group PLC
     * *VOD.GB/GBX*
     */
    vodgbgbx = "vodgbgbx",
    /**
     * ### Voestalpine AG
     * *VOE.AT/EUR*
     */
    voeateur = "voeateur",
    /**
     * ### Volatility Index
     * *VOL.IDX/USD*
     */
    volidxusd = "volidxusd",
    /**
     * ### Controladora Vuela Comp de Aviacion
     * *VOLARA.MX/MXN*
     */
    volaramxmxn = "volaramxmxn",
    /**
     * ### Volvo AB
     * *VOLVB.SE/SEK*
     */
    volvbsesek = "volvbsesek",
    /**
     * ### Volkswagen AG
     * *VOW3.DE/EUR*
     */
    vow3deeur = "vow3deeur",
    /**
     * ### VOYA FINANCIAL INC
     * *VOYA.US/USD*
     */
    voyaususd = "voyaususd",
    /**
     * ### Koninklijke Vopak NV
     * *VPK.NL/EUR*
     */
    vpknleur = "vpknleur",
    /**
     * ### VERTEX PHARMACEUTICALS INC
     * *VRTX.US/USD*
     */
    vrtxususd = "vrtxususd",
    /**
     * ### VISTRA CORP
     * *VST.US/USD*
     */
    vstususd = "vstususd",
    /**
     * ### VIATRIS INC
     * *VTRS.US/USD*
     */
    ualususd = "ualususd",
    /**
     * ### Vestas Wind Systems A/S
     * *VWS.DK/DKK*
     */
    vwsdkdkk = "vwsdkdkk",
    /**
     * ### iPath S&P 500 VIX ST Futures ETN
     * *VXX.US/USD*
     */
    vxxususd = "vxxususd",
    /**
     * ### VERIZON COMMUNICATIONS INC
     * *VZ.US/USD*
     */
    vzususd = "vzususd",
    /**
     * ### WAYFAIR INC- CLASS A
     * *W.US/USD*
     */
    wususd = "wususd",
    /**
     * ### WABTEC CORP
     * *WAB.US/USD*
     */
    wabususd = "wabususd",
    /**
     * ### WESTERN ALLIANCE BANCORP
     * *WAL.US/USD*
     */
    walususd = "walususd",
    /**
     * ### Walmart De Mexico SAB
     * *WALMEX.MX/MXN*
     */
    walmexmxmxn = "walmexmxmxn",
    /**
     * ### WALGREENS BOOTS ALLIANCE INC
     * *WBA.US/USD*
     */
    wbaususd = "wbaususd",
    /**
     * ### WeBuild Spa
     * *WBD.IT/EUR*
     */
    wbditeur = "wbditeur",
    /**
     * ### WEBSTER FINANCIAL CORP
     * *WBS.US/USD*
     */
    wbsususd = "wbsususd",
    /**
     * ### WORKDAY INC-CLASS A
     * *WDAY.US/USD*
     */
    wdayususd = "wdayususd",
    /**
     * ### WESTERN DIGITAL CORP
     * *WDC.US/USD*
     */
    wdcususd = "wdcususd",
    /**
     * ### Weir Group PLC/The
     * *WEIR.GB/GBX*
     */
    weirgbgbx = "weirgbgbx",
    /**
     * ### WENDY'S CO/THE
     * *WEN.US/USD*
     */
    wenususd = "wenususd",
    /**
     * ### WEX INC
     * *WEX.US/USD*
     */
    wexususd = "wexususd",
    /**
     * ### WELLS FARGO & CO
     * *WFC.US/USD*
     */
    wfcususd = "wfcususd",
    /**
     * ### WYNDHAM HOTELS & RESORTS INC
     * *WH.US/USD*
     */
    whususd = "whususd",
    /**
     * ### WHIRLPOOL CORP
     * *WHR.US/USD*
     */
    whrususd = "whrususd",
    /**
     * ### Wolters Kluwer NV
     * *WKL.NL/EUR*
     */
    wklnleur = "wklnleur",
    /**
     * ### WESTLAKE CORP
     * *WLK.US/USD*
     */
    wlkususd = "wlkususd",
    /**
     * ### WAL-MART STORES INC
     * *WMT.US/USD*
     */
    wmtususd = "wmtususd",
    /**
     * ### Wolseley PLC
     * *WOS.GB/GBX*
     */
    wosgbgbx = "wosgbgbx",
    /**
     * ### WP CAREY INC
     * *WPC.US/USD*
     */
    wpcususd = "wpcususd",
    /**
     * ### WPP PLC
     * *WPP.GB/GBX*
     */
    wppgbgbx = "wppgbgbx",
    /**
     * ### WR BERKLEY CORP
     * *WRB.US/USD*
     */
    wrbususd = "wrbususd",
    /**
     * ### WESTROCK CO
     * *WRK.US/USD*
     */
    wrkususd = "wrkususd",
    /**
     * ### WILLIAMS-SONOMA INC
     * *WSM.US/USD*
     */
    wsmususd = "wsmususd",
    /**
     * ### WATSCO INC
     * *WSO.US/USD*
     */
    wsoususd = "wsoususd",
    /**
     * ### WEST PHARMACEUTICAL SERVICES
     * *WST.US/USD*
     */
    wstususd = "wstususd",
    /**
     * ### Whitbread PLC
     * *WTB.GB/GBX*
     */
    wtbgbgbx = "wtbgbgbx",
    /**
     * ### ESSENTIAL UTILITIES INC
     * *WTRG.US/USD*
     */
    wtrgususd = "wtrgususd",
    /**
     * ### WILLIS TOWERS WATSON PLC
     * *WTW.US/USD*
     */
    wtwususd = "wtwususd",
    /**
     * ### WOODWARD INC
     * *WWD.US/USD*
     */
    wwdususd = "wwdususd",
    /**
     * ### WYNN RESORTS LTD
     * *WYNN.US/USD*
     */
    wynnususd = "wynnususd",
    /**
     * ### UNITED STATES STEEL CORP
     * *X.US/USD*
     */
    xususd = "xususd",
    /**
     * ### Spot silver
     * *XAG/USD*
     */
    xagusd = "xagusd",
    /**
     * ### Spot gold
     * *XAU/USD*
     */
    xauusd = "xauusd",
    /**
     * ### Energy Select Sector SPDR Fund
     * *XLE.US/USD*
     */
    xleususd = "xleususd",
    /**
     * ### Financial Select Sector SPDR Fund
     * *XLF.US/USD*
     */
    xlfususd = "xlfususd",
    /**
     * ### Industrial Select Sector SPDR Fund
     * *XLI.US/USD*
     */
    xliususd = "xliususd",
    /**
     * ### Technology Select Sector SPDR Fund
     * *XLK.US/USD*
     */
    xlkususd = "xlkususd",
    /**
     * ### Stellar vs US Dollar
     * *XLM/USD*
     */
    xlmusd = "xlmusd",
    /**
     * ### XILINX INC
     * *XLNX.US/USD*
     */
    xlnxususd = "xlnxususd",
    /**
     * ### Consumer Staples Select Sector SPDR Fund
     * *XLP.US/USD*
     */
    xlpususd = "xlpususd",
    /**
     * ### Utilities Select Sector SPDR Fund
     * *XLU.US/USD*
     */
    xluususd = "xluususd",
    /**
     * ### Health Care Select Sector SPDR Fund
     * *XLV.US/USD*
     */
    xlvususd = "xlvususd",
    /**
     * ### Consumer Discretionary Select Sector SPDR Fund
     * *XLY.US/USD*
     */
    xlyususd = "xlyususd",
    /**
     * ### EXXON MOBIL CORP
     * *XOM.US/USD*
     */
    xomususd = "xomususd",
    /**
     * ### SPDR S&P Oil & Gas Explor & Prodtn ETF
     * *XOP.US/USD*
     */
    xopususd = "xopususd",
    /**
     * ### Palladium
     * *XPD.CMD/USD*
     */
    xpdcmdusd = "xpdcmdusd",
    /**
     * ### Xpeng Inc ADR
     * *XPEV.US/USD*
     */
    xpevususd = "xpevususd",
    /**
     * ### XPO LOGISTICS INC
     * *XPO.US/USD*
     */
    xpoususd = "xpoususd",
    /**
     * ### Platinum
     * *XPT.CMD/USD*
     */
    xptcmdusd = "xptcmdusd",
    /**
     * ### XYLEM INC
     * *XYL.US/USD*
     */
    xylususd = "xylususd",
    /**
     * ### ALLEGHANY CORP
     * *Y.US/USD*
     */
    yususd = "yususd",
    /**
     * ### Yearn.finance vs US Dollar
     * *YFI/USD*
     */
    yfiusd = "yfiusd",
    /**
     * ### YUM! BRANDS INC
     * *YUM.US/USD*
     */
    yumususd = "yumususd",
    /**
     * ### YUM CHINA HOLDINGS INC
     * *YUMC.US/USD*
     */
    yumcususd = "yumcususd",
    /**
     * ### ZILLOW GROUP INC - C
     * *Z.US/USD*
     */
    zususd = "zususd",
    /**
     * ### South African Rand vs Japanese Yen
     * *ZAR/JPY*
     */
    zarjpy = "zarjpy",
    /**
     * ### ZIMMER BIOMET HOLDINGS INC
     * *ZBH.US/USD*
     */
    zbhususd = "zbhususd",
    /**
     * ### ZEBRA TECHNOLOGIES CORP-CL A
     * *ZBRA.US/USD*
     */
    zbraususd = "zbraususd",
    /**
     * ### ZEN.US/USD
     * *ZEN.US/USD*
     */
    zenususd = "zenususd",
    /**
     * ### Zoom Video Communications (A Class)
     * *ZM.US/USD*
     */
    zmususd = "zmususd",
    /**
     * ### ZSCALER INC
     * *ZS.US/USD*
     */
    zsususd = "zsususd",
    /**
     * ### ZOETIS INC
     * *ZTS.US/USD*
     */
    ztsususd = "ztsususd",
    /**
     * ### Zurich Insurance Group AG
     * *ZURN.CH/CHF*
     */
    zurnchchf = "zurnchchf"
}
declare type InstrumentType = keyof typeof Instrument;

declare const VolumeUnit: {
    /**
     * A single unit of a currency, commodity, or security.
     */
    readonly units: "units";
    /**
     * A thousand units of a currency, commodity, or security.
     */
    readonly thousands: "thousands";
    /**
     * A million units of a currency, commodity, or security.
     */
    readonly millions: "millions";
};
declare type VolumeUnitType = keyof typeof VolumeUnit;

declare type OptionalKeys<T> = {
    [K in keyof T]-?: {} extends Pick<T, K> ? K : never;
}[keyof T];
declare type DateInput = string | Date | number;
interface ConfigBase {
    instrument: InstrumentType;
    dates: {
        from: DateInput;
        to: DateInput;
    };
    timeframe?: TimeframeType;
    priceType?: PriceType;
    utcOffset?: number;
    volumes?: boolean;
    volumeUnits?: VolumeUnitType;
    ignoreFlats?: boolean;
    format?: FormatType;
    batchSize?: number;
    pauseBetweenBatchesMs?: number;
    useCache?: boolean;
    cacheFolderPath?: string;
    retryCount?: number;
    retryOnEmpty?: boolean;
    failAfterRetryCount?: boolean;
    pauseBetweenRetriesMs?: number;
}
declare type DefaultConfig = Required<Pick<ConfigBase, OptionalKeys<ConfigBase>>>;
declare const defaultConfig: DefaultConfig;
interface ConfigArrayTickItem extends ConfigBase {
    timeframe?: 'tick';
    format?: 'array';
}
interface ConfigArrayItem extends ConfigBase {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'array';
}
interface ConfigJsonTickItem extends ConfigBase {
    timeframe?: 'tick';
    format?: 'json';
}
interface ConfigJsonItem extends ConfigBase {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'json';
}
interface ConfigCsvItem extends ConfigBase {
    format?: 'csv';
}
declare type Config = ConfigArrayTickItem | ConfigArrayItem | ConfigJsonTickItem | ConfigJsonItem | ConfigCsvItem;

declare type InputSchema<Input> = Record<keyof Required<Input>, ValidationRule>;
declare const schema: InputSchema<Config>;

declare type ConfigValidationError = Pick<ValidationError, 'message' | 'expected' | 'actual'>;
declare type SanitizedConfig<InputConfig extends ConfigBase> = Required<Omit<InputConfig, 'dates'>> & {
    dates: {
        from: Date;
        to: Date;
    };
};
interface ValidateConfigOutput<InputConfig extends ConfigBase> {
    input: SanitizedConfig<InputConfig>;
    isValid: boolean;
    validationErrors: ConfigValidationError[];
}
/**
 *
 * Validates the config and sanitizes some of the params
 *
 */
declare function validateConfig<InputConfig extends ConfigBase>(input: InputConfig, schema: InputSchema<InputConfig>): ValidateConfigOutput<InputConfig>;
declare function validateConfigNode(input: Config): ValidateConfigOutput<Config>;

interface NormaliseDatesInput {
    instrument: InstrumentType;
    startDate: Date;
    endDate: Date;
    timeframe: TimeframeType;
    utcOffset: number;
}

declare function normaliseDates({ instrument, startDate, endDate, timeframe, utcOffset }: NormaliseDatesInput): [Date, Date];

interface GenerateUrlsInput {
    instrument: InstrumentType;
    timeframe: TimeframeType;
    startDate: Date;
    endDate: Date;
    priceType: PriceType;
    nowDate?: Date;
}

declare const URL_ROOT = "https://datafeed.dukascopy.com/datafeed";
declare function generateUrls({ instrument, timeframe, priceType, startDate, endDate }: GenerateUrlsInput): string[];

interface BufferObject {
    url: string;
    buffer: Buffer;
    isCacheHit?: boolean;
}
interface BufferFetcherInput {
    batchSize?: number;
    pauseBetweenBatchesMs?: number;
    onItemFetch?: (url: string, buffer: Buffer, isCacheHit: boolean) => void;
    onBatchFetch?: (bufferObjects: BufferObject[], isLastBatch: boolean) => Promise<unknown>;
    fetcherFn?: (url: string) => Promise<Buffer>;
    cacheManager?: CacheManagerBase;
    retryCount?: number;
    retryOnEmpty?: boolean;
    failAfterRetryCount?: boolean;
    pauseBetweenRetriesMs?: number;
}

declare type CacheKeyFormatter = (input: string) => string;
interface CacheManagerBase {
    readItemFromCache(item: string): Promise<Buffer | null>;
    writeItemsToCache(items: BufferObject[]): Promise<PromiseSettledResult<void>[]>;
    purgeCache(): Promise<void>;
    getCacheKeyFromUrl: CacheKeyFormatter;
}
declare class CacheManager implements CacheManagerBase {
    private cacheManifest;
    private cacheFolderPath;
    cacheKeyFormatter: CacheKeyFormatter;
    constructor({ cacheFolderPath, cacheKeyFormatter }: {
        cacheFolderPath?: string;
        cacheKeyFormatter?: CacheKeyFormatter;
    });
    readItemFromCache(key: string): Promise<Buffer | null>;
    writeItemsToCache(items: BufferObject[]): Promise<PromiseSettledResult<void>[]>;
    purgeCache(cacheFolderPath?: string): Promise<void>;
    getCacheKeyFromUrl(key: string): string;
}

declare class BufferFetcher {
    batchSize: number;
    pauseBetweenBatchesMs: number;
    onItemFetch?: BufferFetcherInput['onItemFetch'];
    onBatchFetch?: BufferFetcherInput['onBatchFetch'];
    fetcherFn: ((url: string) => Promise<Buffer>) | undefined;
    retryCount: number;
    retryOnEmpty: boolean;
    pauseBetweenRetriesMs: number;
    failAfterRetryCount: BufferFetcherInput['failAfterRetryCount'];
    cacheManager?: CacheManagerBase;
    constructor({ batchSize, pauseBetweenBatchesMs, onItemFetch, onBatchFetch, fetcherFn, retryCount, retryOnEmpty, failAfterRetryCount, pauseBetweenRetriesMs, cacheManager }: BufferFetcherInput);
    private fetchBatch;
    /**
     * @experimental
     */
    fetch_optimized(urls: string[]): Promise<true>;
    fetch(urls: string[]): Promise<BufferObject[]>;
    private fetchBuffer;
}

interface ProcessDataInput {
    instrument: InstrumentType;
    requestedTimeframe: TimeframeType;
    bufferObjects: BufferObject[];
    priceType: PriceType;
    volumes: boolean;
    volumeUnits: VolumeUnitType;
    ignoreFlats: boolean;
}
declare type ProcessDataOutput = number[][];

declare function processData({ instrument, requestedTimeframe, bufferObjects, priceType, volumes, volumeUnits, ignoreFlats }: ProcessDataInput): ProcessDataOutput;

declare type Timestamp = number;
declare type AskPrice = number;
declare type BidPrice = number;
declare type Volume = number;
declare type AskVolume = number;
declare type BidVolume = number;
declare type OpenPrice = number;
declare type HighPrice = number;
declare type LowPrice = number;
declare type ClosePrice = number;
interface JsonItem {
    /**
     * UNIX timestamp of the timeframe (candle open)
     */
    timestamp: Timestamp;
    /**
     * Open price of the timeframe (candle)
     */
    open: OpenPrice;
    /**
     * Highest price of the timeframe (candle)
     */
    high: HighPrice;
    /**
     * Lowest price of the timeframe (candle)
     */
    low: LowPrice;
    /**
     * Close price of the timeframe (candle)
     */
    close: ClosePrice;
    /**
     * Traded volume within the timeframe (candle)
     */
    volume?: Volume;
}
interface JsonItemTick {
    /**
     * UNIX timestamp of the tick (price change)
     */
    timestamp: Timestamp;
    /**
     * Ask price of the tick
     */
    askPrice: AskPrice;
    /**
     * Bid price of the tick
     */
    bidPrice: BidPrice;
    /**
     * Ask volume of the tick
     */
    askVolume?: AskVolume;
    /**
     * Bid volume of the tick
     */
    bidVolume?: BidVolume;
}
declare type ArrayItem = [Timestamp, OpenPrice, HighPrice, LowPrice, ClosePrice, Volume?];
declare type ArrayTickItem = [Timestamp, AskPrice, BidPrice, AskVolume?, BidVolume?];
declare type Output = JsonItem[] | JsonItemTick[] | string | ArrayItem[] | ArrayTickItem[];

declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: 'tick';
    format: 'array';
}): ArrayTickItem[];
declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: 'tick';
    format: 'json';
}): JsonItemTick[];
declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: Exclude<TimeframeType, 'tick'>;
    format: 'json';
}): JsonItem[];
declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: Exclude<TimeframeType, 'tick'>;
    format: 'array';
}): ArrayItem[];
declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: TimeframeType;
    format: 'csv';
}): string;
declare function formatOutput({ processedData, timeframe, format }: {
    processedData: ProcessDataOutput;
    timeframe: TimeframeType;
    format: FormatType;
}): Output;

declare function getHistoricalRates(config: ConfigArrayItem): Promise<ArrayItem[]>;
declare function getHistoricalRates(config: ConfigArrayTickItem): Promise<ArrayTickItem[]>;
declare function getHistoricalRates(config: ConfigJsonItem): Promise<JsonItem[]>;
declare function getHistoricalRates(config: ConfigJsonTickItem): Promise<JsonItemTick[]>;
declare function getHistoricalRates(config: ConfigCsvItem): Promise<string>;
declare function getHistoricalRates(config: Config): Promise<Output>;
/**
 * @deprecated use `getHistoricalRates` instead
 *
 * Preserving for backwards compatibility
 * @see https://github.com/Leo4815162342/dukascopy-node/pull/73
 */
declare const getHistoricRates: typeof getHistoricalRates;

declare type CurrentRatesConfigBase = {
    instrument: InstrumentType;
    timeframe?: TimeframeType;
    dates?: {
        from: DateInput;
        to?: DateInput;
    };
    limit?: number;
    volumes?: boolean;
    format?: FormatType;
    priceType?: PriceType;
};
declare type CurrentRatesConfigArrayItem = CurrentRatesConfigBase & {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'array';
};
declare type CurrentRatesConfigArrayTickItem = CurrentRatesConfigBase & {
    timeframe?: 'tick';
    format?: 'array';
};
declare type CurrentRatesConfigJsonItem = CurrentRatesConfigBase & {
    timeframe?: Exclude<TimeframeType, 'tick'>;
    format?: 'json';
};
declare type CurrentRatesConfigJsonTickItem = CurrentRatesConfigBase & {
    timeframe?: 'tick';
    format?: 'json';
};
declare type CurrentRatesConfigCsv = CurrentRatesConfigBase & {
    timeframe?: TimeframeType;
    format?: 'csv';
};
declare type CurrentRatesConfig = CurrentRatesConfigArrayItem | CurrentRatesConfigArrayTickItem | CurrentRatesConfigJsonItem | CurrentRatesConfigJsonTickItem | CurrentRatesConfigCsv;
declare function getCurrentRates(config: CurrentRatesConfigArrayItem): Promise<ArrayItem[]>;
declare function getCurrentRates(config: CurrentRatesConfigArrayTickItem): Promise<ArrayTickItem[]>;
declare function getCurrentRates(config: CurrentRatesConfigJsonItem): Promise<JsonItem[]>;
declare function getCurrentRates(config: CurrentRatesConfigJsonTickItem): Promise<JsonItemTick[]>;
declare function getCurrentRates(config: CurrentRatesConfigCsv): Promise<string>;

export { ArrayItem, ArrayTickItem, BufferFetcher, CacheManager, Config, ConfigValidationError, CurrentRatesConfig, CurrentRatesConfigArrayItem, CurrentRatesConfigArrayTickItem, CurrentRatesConfigCsv, CurrentRatesConfigJsonItem, CurrentRatesConfigJsonTickItem, DefaultConfig, Format, FormatType, InputSchema, Instrument, InstrumentType, JsonItem, JsonItemTick, Output, Price, PriceType, SanitizedConfig, Timeframe, TimeframeType, URL_ROOT, ValidateConfigOutput, defaultConfig, formatOutput, generateUrls, getCurrentRates, getHistoricRates, getHistoricalRates, normaliseDates, processData, schema, validateConfig, validateConfigNode };
