...
var instrument_meta_data_default = {
  ...
  "4063jpjpy": _063jpjpy,
  "4452jpjpy": _452jpjpy,
  "4502jpjpy": _502jpjpy2,
  "4503jpjpy": _503jpjpy2,
  "4507jpjpy": _507jpjpy,
  "4523jpjpy": _523jpjpy,
  "4543jpjpy": _543jpjpy,
  "4689jpjpy": _689jpjpy,
  ...
  "5401jpjpy": _401jpjpy,
  "6098jpjpy": _098jpjpy,
  "6301jpjpy": _301jpjpy2,
  "6367jpjpy": _367jpjpy,
  "6501jpjpy": _501jpjpy,
  "6502jpjpy": _502jpjpy3,
  "6503jpjpy": _503jpjpy3,
  "6506jpjpy": _506jpjpy,
  "6702jpjpy": _702jpjpy,
  "6752jpjpy": _752jpjpy,
  "6758jpjpy": _758jpjpy,
...
function getStructFormat(timeframe) {
  return timeframe === "tick" /* tick */ ? ">3i2f" : ">5i1f";
}
...
function getMinuteOHLCfromTicks(ticks, priceType, startTs, volumes) {
  ...
  const ohlc = ticksByMinute.map((data, i) => data.length > 0 ? ticksToOHLC({ ticks: data, priceType, startTs: startTs + i * 1e3 * 60, volumes }) : []);
  return ohlc;
}
function getSecondOHLCfromTicks(ticks, priceType, startTs, volumes) {
  const ticksBySecond = breakdownByInterval(ticks, 3600, (d) => d.getUTCMinutes() * 60 + d.getUTCSeconds());
  const ohlc = ticksBySecond.map((data, i) => data.length > 0 ? ticksToOHLC({ ticks: data, priceType, startTs: startTs + i * 1e3, volumes }) : []);
  return ohlc;
}
...
function aggregate({
  ...
  if (fromTimeframe === toTimeframe) {
    return splitArrayInChunks(data, 1).map((d) => getOHLC({ input: d, filterFlats: ignoreFlats, volumes }));
  } else {
    if (fromTimeframe === "tick" /* tick */) {
      if (toTimeframe === "s1" /* s1 */) {
        ...
        if (toTimeframe === "m5" /* m5 */) {
          return splitArrayInChunks(minuteOHLC, 5).map((d, i) => getOHLC({
            input: d,
        ...
        if (toTimeframe === "m15" /* m15 */) {
          return splitArrayInChunks(minuteOHLC, 15).map((d, i) => getOHLC({
            input: d,
            filterFlats: ignoreFlats,
        ...
        if (toTimeframe === "m30" /* m30 */) {
          return splitArrayInChunks(minuteOHLC, 30).map((d, i) => getOHLC({
            input: d,
            filterFlats: ignoreFlats,
        ...
        if (toTimeframe === "h1" /* h1 */) {
          return [minuteOHLC].map((d, i) => getOHLC({
            input: d,
            filterFlats: ignoreFlats,
    ...
    if (fromTimeframe === "m1" /* m1 */) {
      if (toTimeframe === "m5" /* m5 */) {
        return splitArrayInChunks(data, 5).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
          startTs: startTs + i * 5 * 1e3 * 60,
          volumes
      ...
      if (toTimeframe === "m15" /* m15 */) {
        return splitArrayInChunks(data, 15).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
          startTs: startTs + i * 15 * 1e3 * 60,
          volumes
      ...
      if (toTimeframe === "m30" /* m30 */) {
        return splitArrayInChunks(data, 30).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
          startTs: startTs + i * 30 * 1e3 * 60,
          volumes
      ...
      if (toTimeframe === "h1" /* h1 */) {
        return splitArrayInChunks(data, 60).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
          startTs: startTs + i * 60 * 1e3 * 60,
          volumes
      ...
      if (toTimeframe === "h4" /* h4 */) {
        return splitArrayInChunks(data, 240).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
    ...
    if (fromTimeframe === "h1" /* h1 */) {
      if (toTimeframe === "h4" /* h4 */) {
        return splitArrayInChunks(data, 4).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
          startTs: startTs + i * 240 * 1e3 * 60,
          volumes
      ...
      if (toTimeframe === "d1" /* d1 */) {
        return splitArrayInChunks(data, 24).map((d, i) => getOHLC({
          input: d,
          filterFlats: ignoreFlats,
...
var CacheManager = class {
  ...
  async writeItemsToCache(items) {
    return Promise.allSettled(items.map(({ buffer, url }) => {
...
async function getHistoricalRates(config) {
  ...
  const onItemFetch = process.env.DEBUG ? (url, buffer, isCacheHit) => {
    (0, import_debug.default)(`${DEBUG_NAMESPACE}:fetcher`)(url, `| ${formatBytes(buffer.length)} |`, `${isCacheHit ? "cache" : "network"}`);
  } : void 0;
  ...
  (0, import_debug.default)(`${DEBUG_NAMESPACE}:data`)(`Generated ${filteredData.length} ${timeframe === "tick" /* tick */ ? "ticks" : "OHLC candles"}`);
...
async function getCurrentRates({
  ...
  while (targetTimestamp > +fromDate && shouldFetch) {
    ...
    let fetchedRates = [];
    try {
      const rawResponse = await (0, import_node_fetch2.default)(url, {
        headers: {
